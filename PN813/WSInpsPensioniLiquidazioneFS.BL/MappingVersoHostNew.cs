...
namespace INPS.Pensioni.LiquidazioneFs
{
    public class MappingVersoHostNew
    {
        ...
        public static void ValorizzaAnagrafica(string matricolaOperatore, GestionePensione.DatiPensione datiPensione, Utility.TipoFondo? tipoFondo,
            GestioneLavorazione.DatiLavorazione datiLavorazione, ref Data.FSPL_FSRCNew AreaCalcolo,
            ...
            out GestioneAnagrafica.DatiAnagrafici datiAnagraficiTitolare, out GestioneAnagrafica.DatiAnagrafici datiAnagraficiDelegato, out GestioneAnagrafica.DatiAnagrafici datiAnagraficiTutore, out List<GestioneMiglioramentiContrattuali.DatiQuoteMiglioramentiContrattuali> datiQuoteMiglioramentiContrattuali)
        {
            datiPagamento = null;
            ...
            List<GestioneFondo.DatiFondoPM> listaDatiFondoPM = null;
            datiAnagraficiDelegato = null;
            datiAnagraficiTutore = null;
            datiQuoteMiglioramentiContrattuali = null;
            ...
            if (tipoFondo.HasValue)
            {
                switch (tipoFondo)
                {
                    ...
                    case Utility.TipoFondo.PM:
                        ...
                        GestioneFondo.GetFondoPMRecordFondoByIdPensione(datiPensione.Id, out listaDatiFondoPM);
                        objectFondoXX = listaDatiFondoPM;
                        //prendo il primo che serve più avanti in questo codice
                        datiFondoPM = listaDatiFondoPM.FirstOrDefault();
                        break;
                }
            }
            ...
            if (datiAnagraficiTitolare != null)
            {
                ...
                if (datiPensione.TipoCalcolo.HasValue && (!Utility.isDomandaGiornalistiDipendentiConSistemaPrivato(datiPensione) || categoriaFondoPI != null))
                {
                ...
                if (tipoFondo.HasValue)
                {
                    switch (tipoFondo.Value)
                    {
                        ...
                        case Utility.TipoFondo.PI:
                        case Utility.TipoFondo.PL:
                            if (listaDatiFondoPI != null && listaDatiFondoPI.Count > 0)
                            {
                                anagrafica.TRACODSI = listaDatiFondoPI.FirstOrDefault().Requisiti247_243.HasValue ? listaDatiFondoPI.FirstOrDefault().Requisiti247_243.Value ? "S" : "N" : "";
                                anagrafica.TRAANZ247 = listaDatiFondoPI.FirstOrDefault().AnzianitaAnni.HasValue ? listaDatiFondoPI.FirstOrDefault().AnzianitaAnni.Value.ToString().PadLeft(2, '0') : "00";
                                //TRARECUP occorre passarlo al calcolo sempre a zero per le PL
                                if (tipoDomanda == Utility.TipoDomanda.Ricostituzione || isRiaperturaDomanda)
                                {
                                    anagrafica.TRARECUP = int.Parse((listaDatiFondoPI.FirstOrDefault().AnnoRequisiti.HasValue ? listaDatiFondoPI.FirstOrDefault().AnnoRequisiti.Value.ToString().PadLeft(4, '0') : "0000") +
                                      (listaDatiFondoPI.FirstOrDefault().NumeroTriSemRequisiti.HasValue ? listaDatiFondoPI.FirstOrDefault().NumeroTriSemRequisiti.Value.ToString() : "0"));
                                }
                            }
                            break;
                        case Utility.TipoFondo.GAS:
                ...
                //Utility.CategoriaFondoPI? categoriaFondoPI = Utility.GetCategoriaFondoPI(Utility.TipoAppartenenza.FS, datiPensione.SiglaCategoria);
                //if(Utility.IsRicostituzione(datiPensione.Gruppo) && categoriaFondoPI != null && datiPensione.SiglaCategoria.StartsWith("S") && datiDanteCausa != null && datiDanteCausa.DecorrenzaPensione.HasValue)
                //{
                //    List<GestioneFondo.DatiAgoPI> lstDatiAgoPI;
                //    GestioneFondo.GetListaDatiAgoPIByIdPensione(datiPensione.Id, out lstDatiAgoPI);
                //    if (lstDatiAgoPI != null && lstDatiAgoPI.Count > 0)
                //    {
                //        GestioneFondo.DatiAgoPI datiAgoMin = lstDatiAgoPI.Where(x => x.DecorrenzaDatiAgo.HasValue).OrderBy(x => x.DecorrenzaDatiAgo.Value).FirstOrDefault();
                //        GestioneFondo.DatiAgoPI datiAgoMax = lstDatiAgoPI.Where(x => x.DecorrenzaDatiAgo.HasValue).OrderByDescending(x => x.DecorrenzaDatiAgo.Value).FirstOrDefault();

                //        anagrafica.TRADIAAA = datiAgoMin != null && datiAgoMin.DecorrenzaDatiAgo.HasValue ? (short)datiAgoMin.DecorrenzaDatiAgo.Value.Year : anagrafica.TRADIAAA;
                //        anagrafica.TRADIAMM = datiAgoMin != null && datiAgoMin.DecorrenzaDatiAgo.HasValue ? (short)datiAgoMin.DecorrenzaDatiAgo.Value.Month : anagrafica.TRADIAAA;

                //        anagrafica.TRASPAAA = datiAgoMax != null && datiAgoMax.DecorrenzaDatiAgo.HasValue ? (short)datiAgoMax.DecorrenzaDatiAgo.Value.Year : anagrafica.TRASPAAA;
                //        anagrafica.TRASPAMM = datiAgoMax != null && datiAgoMax.DecorrenzaDatiAgo.HasValue ? (short)datiAgoMax.DecorrenzaDatiAgo.Value.Month : anagrafica.TRASPAMM;
                //    }
                //}
                ...
                //Utility.CategoriaFondoPI? categoriaFondoPI = Utility.GetCategoriaFondoPI(Utility.TipoAppartenenza.FS, datiPensione.SiglaCategoria);
                //REV e RIC di REV
                //campo del calcolo (es. 199107) DECDIRAGO-A 🡪decorrenza relativa alla prima occorrenza del record ago
                //campo del prelievo TRADIAAA, TRADIAMM
                //campo del calcolo(es. 199302) DECSUPAGO - A 🡪 mese successivo al mese della morte del dante causa --> sarebbe la decorrenza della pensione
                // Campo del prelievo TRASPAAA,TRASPAMM
                if (categoriaFondoPI != null && datiPensione.SiglaCategoria.StartsWith("S") && datiDanteCausa != null && datiDanteCausa.DecorrenzaPensione.HasValue && datiDanteCausa.Certificato.HasValue)
                {
                    List<GestioneFondo.DatiAgoPI> lstDatiAgoPI;
                    GestioneFondo.GetListaDatiAgoPIByIdPensione(datiPensione.Id, out lstDatiAgoPI);
                    if (lstDatiAgoPI != null && lstDatiAgoPI.Count > 0)
                    {
                        ...
                        anagrafica.TRASPAAA = datiPensione.DecorrenzaOriginaria.HasValue ? (short)datiPensione.DecorrenzaOriginaria.Value.Year : (short)0;
                        anagrafica.TRASPAMM = datiPensione.DecorrenzaOriginaria.HasValue ? (short)datiPensione.DecorrenzaOriginaria.Value.Month : (short)0;
                    }
                }

                //RIC non superstiti
                //campo del calcolo(es. 199107) DECDIRAGO - A 🡪 decorrenza relativa alla prima occorrenza del record ago, se presente
                //TRADIAAA,  TRADIAMM
                if (Utility.IsRicostituzione(datiPensione.Gruppo) && categoriaFondoPI != null && !datiPensione.SiglaCategoria.StartsWith("S"))
                {
                    List<GestioneFondo.DatiAgoPI> lstDatiAgoPI;
                    GestioneFondo.GetListaDatiAgoPIByIdPensione(datiPensione.Id, out lstDatiAgoPI);
                    if (lstDatiAgoPI != null && lstDatiAgoPI.Count > 0)
                ...
                //PL e RIC di indirette
                //campo del calcolo(es. 199302) DECSUPAGO - A 🡪 mese successivo al mese della morte del dante causa --> sarebbe la decorrenza della pensione
                // Campo del prelievo TRASPAAA,TRASPAMM
                if (categoriaFondoPI != null && datiPensione.SiglaCategoria.StartsWith("S") && datiDanteCausa != null && !datiDanteCausa.Certificato.HasValue)
                {
                    anagrafica.TRASPAAA = datiPensione.DecorrenzaOriginaria.HasValue ? (short)datiPensione.DecorrenzaOriginaria.Value.Year : (short)0;
                    anagrafica.TRASPAMM = datiPensione.DecorrenzaOriginaria.HasValue ? (short)datiPensione.DecorrenzaOriginaria.Value.Month : (short)0;
                }
        ...
        public static void ValorizzaDelegato(GestionePensione.DatiPensione datiPensione, GestionePagamento.DatiPagamento datiPagamento,
            ...
            out Liquidazione.BLCommon.GestioneMaggiorazioniBenefici.DatiMaggiorazioniBenefici datiMaggiorazioniBenefici, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            ...
            if (datiPatronato != null)
            {
                ...
                if (!string.IsNullOrEmpty(datiPatronato.TipoUfficio))
                    delegato.TRBTIPOUFFPAT = datiPatronato.TipoUfficio.Trim() == "02" ? "01" : datiPatronato.TipoUfficio.Trim() == "23" ? "02" : (categoriaFondoPI != null ? datiPatronato.TipoUfficio : "");
        ...
        public static void ValorizzaFamiliareByIdPensione(GestionePensione.DatiPensione datiPensione, out Dictionary<string, char> componentiFamiliari, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
                        ...
                        if (listaCodMaggFamiliariParziali != null && listaCodMaggFamiliariParziali.Count > 0)
                        {
                            ...
                            for (int i = 0; i < listaCodMaggFamiliariParziali.Count; i++)
                            {
                                if (listaCodMaggFamiliariParziali[i].Decorrenza.HasValue || listaCodMaggFamiliariParziali[i].Cessazione.HasValue)
                                {
                                    ...
                                    string siglaCategoria = datiPensione.SiglaCategoria.Trim().ToUpper();
                                    string gruppo = datiPensione.Gruppo.Trim();
                                    if ((siglaCategoria.Equals("SFS") || siglaCategoria.Equals("SPT")) && gruppo.Equals("0031"))
                                    {
                                        trcConti.TRCQUOTA = listaCodMaggFamiliariParziali[i].QuotaAF != null ? listaCodMaggFamiliariParziali[i].QuotaAF : null;
                                        trcConti.TRCCNFON = listaCodMaggFamiliariParziali[i].ContitolaritaFondo != null ? listaCodMaggFamiliariParziali[i].ContitolaritaFondo : null;
                                    }
        ...
        public static void ValorizzaDanteCausaByIdPensione(GestionePensione.DatiPensione datiPensione, GestioneDanteCausa.DatiDanteCausa datiDanteCausa, out GestioneAnagrafica.DatiAnagrafici datiAnagraficiDanteCausa,
            ref Data.FSPL_FSRCNew AreaCalcolo)
        {
        ...
        public static void ValorizzaDelegatoTutore(GestioneAnagrafica.DatiAnagrafici datiAnagraficiDelegato, GestioneAnagrafica.DatiAnagrafici datiAnagraficiTutore, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
        ...
        public static void ValorizzaSupplementi(GestionePensione.DatiPensione datiPensione, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
        ...
        public static void ValorizzaTrattamentiFamiglia(List<GestioneAnagrafica.DatiStatoCivile> listaStatiCivili, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
        ...
        public static void ValorizzaMinimo_PensInv(long idPensione, out Entity.DatiBititolaritaInail datiBititolaritaInail, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
        ...
        public static void ValorizzaResidenza(GestionePensione.DatiPensione datiPensione, Liquidazione.BLCommon.GestioneMaggiorazioniBenefici.DatiMaggiorazioniBenefici datiMaggiorazioniBenefici, Utility.TipoFondo? tipoFondo,
             GestioneIstruttoria.DatiIstruttoria datiIstruttoria, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
        ...
        public static void ValorizzaMaggiorazioneLegge(GestionePensione.DatiPensione datiPensione, out GestioneDL407.DatiDL407 datiDL407, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
        ...
        public static void ValorizzaRenditaINAIL(Entity.DatiBititolaritaInail datiBititolaritaInail, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
        ...
        public static void ValorizzaTrattenuteLavAutonomi(ref Data.FSPL_FSRCNew AreaCalcolo)
        {
        ...
        public static void ValorizzaAgoTeorico(ref Data.FSPL_FSRCNew AreaCalcolo)
        {
        ...
        public static void ValorizzaMaggiorazioneSociale(Liquidazione.BLCommon.GestioneMaggiorazioniBenefici.DatiMaggiorazioniBenefici datiMaggiorazioniBenefici, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            if (datiMaggiorazioniBenefici != null)
            {
        ...
        public static void ValorizzaRecordR(GestionePensione.DatiPensione datiPensione, GestioneAnagrafica.DatiAnagrafici datiAnagraficiTitolare, Utility.TipoFondo? tipoFondo, GestioneFondo.DatiFondo datiFondo, Object objectFondoXX, List<GestioneRecordFondo.DatiRecordFondo> listaRecordFondo, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            ...
            if (datiPensione != null)
            {
                ...
                {
                    redditi.LISGP2BM00_R = new List<Data.CMSGTRA.Redditi.GP2BM00_R>();
                    foreach (GestioneRecordFondo.DatiRecordFondo recordFondo in listaRecordFondo)
                    {
                        Data.CMSGTRA.Redditi.GP2BM00_R redd = new Data.CMSGTRA.Redditi.GP2BM00_R();
                        redd.GP2BM13_R = Convert.ToDecimal(GetSettimaneUtiliOI(datiPensione, tipoFondo, datiFondo, objectFondoXX, recordFondo));
                        redd.GP2BMTA_R = "OI"; // Memo 79
                        redditi.LISGP2BM00_R.Add(redd);
                    }
                }

                //ENG - Implementazione Meta Processo - Per la linea FS valorizzare il campo per tutte le domande
                if (ctrlSbloccaMetaProcesso != null && !String.IsNullOrEmpty(ctrlSbloccaMetaProcesso.ValoreControllo) && ctrlSbloccaMetaProcesso.ValoreControllo.Trim().ToUpperInvariant() == "SI")
                {
                    ...
                    {
                        redditi.SEDE_METAPROCESSO = datiPensione.CodiceSedeLavorazione.Value.ToString().PadLeft(4, '0') + (datiPensione.CentroOperativo.HasValue ? datiPensione.CentroOperativo.GetValueOrDefault().ToString().PadLeft(2, '0') : "00");
                    }
                }
        ...
        private static double GetSettimaneUtiliOI(GestionePensione.DatiPensione datiPensione, Utility.TipoFondo? tipoFondo, GestioneFondo.DatiFondo datiFondo, Object objectFondoXX, GestioneRecordFondo.DatiRecordFondo recordFondo)
        {
            long idRecordFondo = recordFondo.Id;
            double SettimaneFromAAMMGG = 0;
            if (tipoFondo.HasValue)
            {
                switch (tipoFondo)
                {
                    case Utility.TipoFondo.FS:
                        ...
                        GestioneFondo.DatiFondoFST datiFondoFST = lstDatiFondoFST.Find(x => x.IdRecordFondo == idRecordFondo);
                        SettimaneFromAAMMGG = Utility.CalcolaSettimane(datiFondoFST.ServizioUtileDirittoOIAA.Value, datiFondoFST.ServizioUtileDirittoOIMM.Value, datiFondoFST.ServizioUtileDirittoOIGG.Value);
                        break;
                    case Utility.TipoFondo.PT:
                        List<GestioneFondo.DatiFondoPT> lstDatiFondoPT = objectFondoXX as List<GestioneFondo.DatiFondoPT>;
                        GestioneFondo.DatiFondoPT datiFondoPT = lstDatiFondoPT.Find(x => x.IdRecordFondo == idRecordFondo);
                        SettimaneFromAAMMGG = Utility.CalcolaSettimane(datiFondoPT.ServizioUtileDirittoOIAA.Value, datiFondoPT.ServizioUtileDirittoOIMM.Value, datiFondoPT.ServizioUtileDirittoOIGG.Value);
                        break;
                    default:
                        SettimaneFromAAMMGG = datiFondo.SettimaneUtiliDirittoOI.HasValue ? datiFondo.SettimaneUtiliDirittoOI.Value : 0;
                        break;
                }
            }
            else if (Utility.IsDomandaINPDAP(datiPensione.Gestione))
            {
                ...
                SettimaneFromAAMMGG = Utility.CalcolaSettimane(datiRecordFondoINPDAP.ServizioUtileDirittoOIAA.Value, datiRecordFondoINPDAP.ServizioUtileDirittoOIMM.Value, datiRecordFondoINPDAP.ServizioUtileDirittoOIGG.Value);
            }

            return SettimaneFromAAMMGG;
        }

        public static void ValorizzaDatiNonCalcolo(GestionePensione.DatiPensione datiPensione, Dictionary<string, char> componentiFamiliari, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
        ...
        public static void ValorizzaGp4Inpdap(GestionePensione.DatiPensione datiPensione, GestioneAnagrafica.DatiAnagrafici datiAnagraficiTitolare,
            GestioneDanteCausa.DatiDanteCausa datiDanteCausa, GestioneAnagrafica.DatiAnagrafici datiAnagraficiDanteCausa, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            List<GestioneAventiDiritto.AventiDiritto> listaAventiDiritto = null;
            List<GestioneAnagrafica.DatiAnagrafici> listaAnagraficheAventiDiritto = null;
            GestioneAventiDiritto.GetAventiDirittoConAnagraficheByIdPensione(datiPensione.Id, out listaAventiDiritto, out listaAnagraficheAventiDiritto);
        ...
        public static void ValorizzaFondoEL(GestionePensione.DatiPensione datiPensione, Object objectFondoXX,
            ...
            GestioneIstruttoria.DatiIstruttoria datiIstruttoria, GestioneFondo.DatiFondo datiFondo, GestioneDanteCausa.DatiDanteCausa datiDanteCausa, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
        ...
        public static void ValorizzaFondoET(GestionePensione.DatiPensione datiPensione, Object objectFondoXX, GestioneFondo.DatiFondo datiFondo,
            List<GestioneRecordFondo.DatiRecordFondo> listaRecordFondo, Liquidazione.BLCommon.GestioneMaggiorazioniBenefici.DatiMaggiorazioniBenefici datiMaggiorazioniBenefici,
            GestioneIstruttoria.DatiIstruttoria datiIstruttoria, GestioneDanteCausa.DatiDanteCausa datiDanteCausa, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            if (listaRecordFondo != null && listaRecordFondo.Count > 0)
            {
        ...
        public static void ValorizzaFondoVL(Object objectFondoXX, List<GestioneRecordFondo.DatiRecordFondo> listaRecordFondo,
            ...
            GestionePensione.DatiPensione datiPensione, GestioneFondo.DatiFondo datiFondo, GestioneDanteCausa.DatiDanteCausa datiDanteCausa, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            if (listaRecordFondo != null && listaRecordFondo.Count > 0)
            {
                List<GestioneDatiServizioUtile.ServizioUtile> datiServizioUtile = null;
        ...
        public static void ValorizzaFondoPT(GestionePensione.DatiPensione datiPensione, Object objectFondoXX, GestioneFondo.DatiFondo datiFondo,
            List<GestioneRecordFondo.DatiRecordFondo> listaRecordFondo, bool isNuovaGestione, GestioneDanteCausa.DatiDanteCausa datiDanteCausa, GestioneLavorazione.DatiLavorazione datiLavorazione, out List<GestioneDatiServizioUtile.ServizioUtile> datiServizioUtileByIdPensione, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            Utility.TipoDomanda tipoDomanda = Utility.GetTipoDomanda(datiPensione.Gruppo, datiPensione.Prodotto);
                ...
                if (AreaCalcolo.UtilizzaNuovoTracciato)
                {
                    ...
                    for (int i = 0; i < listaRecordFondo.Count; i++)
                    {
                        ...
                        if (datiFondoPT != null)
                        {
                            ...
                            fondoPT.XFSIISL = datiFondoPT.IndennitaIntegrativaSpecialeLorda.HasValue ? (decimal)datiFondoPT.IndennitaIntegrativaSpecialeLorda : 0;
                            ...
                            if (Utility.IsRicostituzioneOrRiapertura(datiPensione, Utility.IsRiaperturaDomanda(datiPensione.Id)) && Utility.IsDomandaReversibilitaOrRicostituzione(datiPensione, datiDanteCausa, datiLavorazione))
                            {
                                fondoPT.XFSFAAGO = datiFondoPT.XFSFAAGO.GetValueOrDefault();
                            }
                        }
                        ...
                        if (datiFondoPT != null)
                        {
                            ...
                            {
                                short Anni, Mesi, Giorni;
                                int AnniOI = datiFondoPT.ServizioUtileDirittoOIAA.HasValue ? datiFondoPT.ServizioUtileDirittoOIAA.Value : (short)0;
                                int MesiOI = datiFondoPT.ServizioUtileDirittoOIMM.HasValue ? datiFondoPT.ServizioUtileDirittoOIMM.Value : (short)0;
                                int GiorniOI = datiFondoPT.ServizioUtileDirittoOIGG.HasValue ? datiFondoPT.ServizioUtileDirittoOIGG.Value : (short)0;

                                Utility.SommaSettimaneAAMMGG(fondoPT.XFSSUAN, fondoPT.XFSSUANMM, fondoPT.XFSSUANGG, AnniOI, MesiOI, GiorniOI, out Anni, out Mesi, out Giorni);

                                fondoPT.XFSSUAN = Anni;
                                fondoPT.XFSSUANMM = Mesi;
                                fondoPT.XFSSUANGG = Giorni;
                            }
        ...
        public static void ValorizzaFondoINPDAP(GestionePensione.DatiPensione datiPensione, Object objectFondoXX, List<GestioneRecordFondo.DatiRecordFondo> listaRecordFondo,
            ...
            out List<GestioneDatiServizioUtileINPDAP.ServizioUtile> datiServizioUtileINPDAPByIdPensione, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            ...
            if (listaRecordFondo != null && listaRecordFondo.Count > 0)
            {
                ...
                foreach (GestioneRecordFondo.DatiRecordFondo recordFondo in listaRecordFondo)
                {
                    ...
                    if (datiRecordFondoINPDAP != null)
                    {
                        ...
                        {
                            short Anni, Mesi, Giorni;
                            int AnniOI = datiRecordFondoINPDAP.ServizioUtileDirittoOIAA.HasValue ? datiRecordFondoINPDAP.ServizioUtileDirittoOIAA.Value : (short)0;
                            int MesiOI = datiRecordFondoINPDAP.ServizioUtileDirittoOIMM.HasValue ? datiRecordFondoINPDAP.ServizioUtileDirittoOIMM.Value : (short)0;
                            int GiorniOI = datiRecordFondoINPDAP.ServizioUtileDirittoOIGG.HasValue ? datiRecordFondoINPDAP.ServizioUtileDirittoOIGG.Value : (short)0;

                            Utility.SommaSettimaneAAMMGG(fondoGDP.SUAN_GDP, fondoGDP.SUAN_MM_GDP, fondoGDP.SUAN_GG_GDP, AnniOI, MesiOI, GiorniOI, out Anni, out Mesi, out Giorni);

                            fondoGDP.SUAN_GDP = Anni;
                            fondoGDP.SUAN_MM_GDP = Mesi;
                            fondoGDP.SUAN_GG_GDP = Giorni;
                        }
        ...
        public static void ValorizzaFondoFS(GestionePensione.DatiPensione datiPensione, Object objectFondoXX,
            List<GestioneRecordFondo.DatiRecordFondo> listaRecordFondo, INPS.Pensioni.Liquidazione.BLCommon.GestioneMaggiorazioniBenefici.DatiMaggiorazioniBenefici datiMaggiorazioniBenefici,
            GestioneIstruttoria.DatiIstruttoria datiIstruttoria, GestioneFondo.DatiFondo datiFondo, bool isNuovaGestione, GestioneDanteCausa.DatiDanteCausa datiDanteCausa, GestioneLavorazione.DatiLavorazione datiLavorazione, out List<GestioneDatiServizioUtile.ServizioUtile> datiServizioUtileByIdPensione,
            ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            datiServizioUtileByIdPensione = null;
            if (listaRecordFondo != null && listaRecordFondo.Count > 0)
                ...
                if (AreaCalcolo.UtilizzaNuovoTracciato)
                {
                    ...
                    foreach (GestioneRecordFondo.DatiRecordFondo recordFondo in listaRecordFondo)
                    {
                        ...
                        if (datiFondoFST != null)
                        {
                            ...
                            {
                                short Anni, Mesi, Giorni;
                                int AnniOI = datiFondoFST.ServizioUtileDirittoOIAA.HasValue ? datiFondoFST.ServizioUtileDirittoOIAA.Value : (short)0;
                                int MesiOI = datiFondoFST.ServizioUtileDirittoOIMM.HasValue ? datiFondoFST.ServizioUtileDirittoOIMM.Value : (short)0;
                                int GiorniOI = datiFondoFST.ServizioUtileDirittoOIGG.HasValue ? datiFondoFST.ServizioUtileDirittoOIGG.Value : (short)0;

                                Utility.SommaSettimaneAAMMGG(fondoFS.XFSSUAN, fondoFS.XFSSUANMM, fondoFS.XFSSUANGG, AnniOI, MesiOI, GiorniOI, out Anni, out Mesi, out Giorni);

                                fondoFS.XFSSUAN = Anni;
                                fondoFS.XFSSUANMM = Mesi;
                                fondoFS.XFSSUANGG = Giorni;
                            }
                            ...
                            fondoFS.XFSIISL = datiFondoFST.IndennitaIntegrativaSpecialeLorda.HasValue ? (decimal)datiFondoFST.IndennitaIntegrativaSpecialeLorda : 0;
                            ...
                            if (Utility.IsRicostituzioneOrRiapertura(datiPensione, Utility.IsRiaperturaDomanda(datiPensione.Id)) && Utility.IsDomandaReversibilitaOrRicostituzione(datiPensione, datiDanteCausa, datiLavorazione))
                            {
                                fondoFS.XFSFAAGO = datiFondoFST.XFSFAAGO.GetValueOrDefault();
                            }
        ...
        public static void ValorizzaFondoPI(List<GestioneRecordFondo.DatiRecordFondo> listaRecordFondo, INPS.Pensioni.Liquidazione.BLCommon.GestioneMaggiorazioniBenefici.DatiMaggiorazioniBenefici datiMaggiorazioniBenefici,
            GestionePensione.DatiPensione datiPensione, GestioneFondo.DatiFondo datiFondo, Object objectFondoXX, GestioneDanteCausa.DatiDanteCausa datiDanteCausa, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            if (listaRecordFondo != null && listaRecordFondo.Count > 0)
            {
                ...
                foreach (GestioneRecordFondo.DatiRecordFondo recordFondo in listaRecordFondo)
                {
                    ...
                    fondoPI.XPITIPOR = "X";
                    if (datiPensione.SiglaCategoria != null && (datiPensione.SiglaCategoria.Trim() == "IPL" || datiPensione.SiglaCategoria.Trim() == "SPL"))
                        fondoPI.XPIFONDO = "PL";
                    else
                        fondoPI.XPIFONDO = "PI";
                    ...
                    if (datiFondo != null)
                    {
                        ...
                        if (!String.IsNullOrEmpty(datiFondo.AttivitaSvolta) && datiFondo.AttivitaSvolta.Trim() != "")
                        {
                            ...
                            List<GestioneDecodifica.AttivitaSvolta> elencoAttivitaSvolte = null;
                            if (datiPensione.SiglaCategoria != null && (datiPensione.SiglaCategoria.Trim() == "IPL" || datiPensione.SiglaCategoria.Trim() == "SPL"))
                                GestioneDecodifica.GetAttivitaSvoltaByFondo("PL", enteFondo, out elencoAttivitaSvolte);
                            else
                    ...
                    if (objectFondoXX != null)
                    {
                        ...
                        List<GestioneFondo.DatiFondoPI> lstDatiFondoPI = objectFondoXX as List<GestioneFondo.DatiFondoPI>;

                        //per gestire il pregresso: prendo l'idRecordFondo per la nuova gestione o il primo/unico per il pregresso
                        datiFondoPI = lstDatiFondoPI.Find(x => x.IdRecordFondo == idRecordFondo) != null ? lstDatiFondoPI.Find(x => x.IdRecordFondo == idRecordFondo) : lstDatiFondoPI.FirstOrDefault();
                        ...
                        try
                        {
                            if (categoriaFondoPI.HasValue && categoriaFondoPI.Value == Utility.CategoriaFondoPI.U)
                            {
                                ...
                                string sede = datiFondoPI.Sede.HasValue ? datiFondoPI.Sede.ToString().PadLeft(3, '0').Substring(0, 4) : "0000";
                                string certificato = datiFondoPI.Certificato.HasValue ? datiFondoPI.Certificato.ToString().PadLeft(8, '0').Substring(0, 8) : "00000000";
                                ...
                                // 5 byte 0
                                fondoPI.XPIDP346 = decimal.Parse(settimaneEsclusiveDP346 + settimaneInpdai + datiFondoPI.CodiceCategoria.Substring(0, 1)
                                    + "," + datiFondoPI.CodiceCategoria.Substring(1, 2) + sede.Substring(0, 2));

                                fondoPI.XPISANIT = decimal.Parse(sede.Substring(2, 2) + certificato.Substring(0, 4) + "," + certificato.Substring(4, 4));
                            }
                            else
                            {
                                ...
                                fondoPI.XPIDP346 = datiFondoPI.IncrementoDPR346.HasValue ? datiFondoPI.IncrementoDPR346.Value : 0;
                                fondoPI.XPISANIT = datiFondoPI.AssPers.HasValue ? (decimal)datiFondoPI.AssPers : (decimal)0;
                            }
                        }
                        catch (Exception)
                        {
                        ...
                        if (Utility.IsPIAPIBAnte99(Utility.TipoAppartenenza.FS, datiPensione, datiDanteCausa))
                        {
                            // cifre decimali
                        ...
                        //rimossa valorizzazione per altre casisitche
                        //else
                        //{
                        //    if (datiFondoPI.DecorrenzaPrescrizione.HasValue)
                        //    {
                        ...
                        {
                            fondoPI.XPIAS762 = datiFondoPI.Ass7_62.HasValue ? (decimal)datiFondoPI.Ass7_62 : (decimal)0;
                        }
                        ...
                        fondoPI.XPITIPRG = datiFondoPI.TipoRegolamento;
                        fondoPI.XPISCATT = datiFondoPI.Scatti;
                        fondoPI.XPIULTSE = datiFondoPI.SedeServ.HasValue ? datiFondoPI.SedeServ.Value : (short)0;
                        fondoPI.XPIFISSE = datiFondoPI.Fisse.HasValue ? datiFondoPI.Fisse.Value : (short)0;
                    }
                    ...
                    if (datiMaggiorazioniBenefici != null)
                    {
                        ...
                        if (listaCodiciMaggExComb != null && listaCodiciMaggExComb.Count > 0)
                        {
                            GestioneDecodifica.CodiceMaggiorazioneExCombattenti codMaggExComb = listaCodiciMaggExComb.Find(x => x.Id == datiMaggiorazioniBenefici.ExCombattente.Value);
                            if (codMaggExComb != null)
                                fondoPI.XPIEXCBT = codMaggExComb.TraduzioneSuGP;
                        }
        ...
        public static void ValorizzaFondoGAS(GestionePensione.DatiPensione datiPensione, Object objectFondoXX, List<GestioneRecordFondo.DatiRecordFondo> listaRecordFondo,
            INPS.Pensioni.Liquidazione.BLCommon.GestioneMaggiorazioniBenefici.DatiMaggiorazioniBenefici datiMaggiorazioniBenefici,
            GestioneFondo.DatiFondo datiFondo, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            List<GestioneDatiServizioUtile.ServizioUtile> datiServizioUtile = null;
            GestioneDatiServizioUtile.GetDatiServizioUtileByIdPensione(datiPensione.Id, out datiServizioUtile);
        ...
        public static void ValorizzaFondoES(GestionePensione.DatiPensione datiPensione, List<GestioneRecordFondo.DatiRecordFondo> listaRecordFondo,
            ...
            Object objectFondoXX, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            List<GestioneDatiServizioUtile.ServizioUtile> datiServizioUtile = null;
            GestioneDatiServizioUtile.GetDatiServizioUtileByIdPensione(datiPensione.Id, out datiServizioUtile);
            ...
            if (listaRecordFondo != null && listaRecordFondo.Count > 0)
            {
                ...
                foreach (GestioneRecordFondo.DatiRecordFondo recordFondo in listaRecordFondo)
                {
                    ...
                    if (datiFondo != null)
                    {
                        ...
                        fondoPM.XPMFISSE = datiFondo.CodiceDirittoQuoteFisse.HasValue ? datiFondo.CodiceDirittoQuoteFisse.Value : (short)0;
                    }
                    ...
                    if (objectFondoXX != null)
                    {
                        ...
                        if (objectFondoXX is List<GestioneFondo.DatiFondoPM>)
                        {
                            List<GestioneFondo.DatiFondoPM> lstDatiFondoPM = objectFondoXX as List<GestioneFondo.DatiFondoPM>;
                            //per gestire il pregresso: prendo l'idRecordFondo per la nuova gestione o il primo/unico per il pregresso
                            datiFondoPM = lstDatiFondoPM.Find(x => x.IdRecordFondo == idRecordFondo) != null ? lstDatiFondoPM.Find(x => x.IdRecordFondo == idRecordFondo) : lstDatiFondoPM.FirstOrDefault();
                        }
                        else
                            datiFondoPM = objectFondoXX as GestioneFondo.DatiFondoPM;
                        ...
                        fondoPM.XPMCONVI = datiFondoPM.ConvenzioneInternazionale.HasValue ? datiFondoPM.ConvenzioneInternazionale.Value.ToString() : "";

                        fondoPM.XPMEFFMM = datiFondoPM.MesiNavigazioneEffettiva.HasValue ? datiFondoPM.MesiNavigazioneEffettiva.Value : (short)0;
                        fondoPM.XPMEFFGG = datiFondoPM.GiorniNavigazioneEffettiva.HasValue ? datiFondoPM.GiorniNavigazioneEffettiva.Value : (short)0;
                        fondoPM.XPMRAPMM = datiFondoPM.MesiRadePilotaggio.HasValue ? datiFondoPM.MesiRadePilotaggio.Value : (short)0;
                        fondoPM.XPMRAPGG = datiFondoPM.GiorniRadePilotaggio.HasValue ? datiFondoPM.GiorniRadePilotaggio.Value : (short)0;
                        fondoPM.XPMTBCMM = datiFondoPM.MesiTBCDS.HasValue ? datiFondoPM.MesiTBCDS.Value : (short)0;
                        fondoPM.XPMTBCGG = datiFondoPM.GiorniTBCDS.HasValue ? datiFondoPM.GiorniTBCDS.Value : (short)0;
                        fondoPM.XPMMALMM = datiFondoPM.MesiMalattia.HasValue ? datiFondoPM.MesiMalattia.Value : (short)0;
                        fondoPM.XPMMALGG = datiFondoPM.GiorniMalattia.HasValue ? datiFondoPM.GiorniMalattia.Value : (short)0;
                        fondoPM.XPMESTMM = datiFondoPM.MesiNavigazioneEE.HasValue ? datiFondoPM.MesiNavigazioneEE.Value : (short)0;
                        fondoPM.XPMESTGG = datiFondoPM.GiorniNavigazioneEE.HasValue ? datiFondoPM.GiorniNavigazioneEE.Value : (short)0;
                        fondoPM.XPMALTMM = datiFondoPM.MesiAltriServizi.HasValue ? datiFondoPM.MesiAltriServizi.Value : (short)0;
                        fondoPM.XPMALTGG = datiFondoPM.GiorniAltriServizi.HasValue ? datiFondoPM.GiorniAltriServizi.Value : (short)0;
                        fondoPM.XPMMILMM = datiFondoPM.MesiNavigazioneMilitare.HasValue ? datiFondoPM.MesiNavigazioneMilitare.Value : (short)0;
                        fondoPM.XPMMILGG = datiFondoPM.GiorniNavigazioneMilitare.HasValue ? datiFondoPM.GiorniNavigazioneMilitare.Value : (short)0;
                        fondoPM.XPMMILDM = datiFondoPM.MesiDoppioMilitare.HasValue ? datiFondoPM.MesiDoppioMilitare.Value : (short)0;
                        fondoPM.XPMMILDG = datiFondoPM.GiorniDoppioMilitare.HasValue ? datiFondoPM.GiorniDoppioMilitare.Value : (short)0;
                        fondoPM.XPMMERMM = datiFondoPM.MesiDoppioMercantile.HasValue ? datiFondoPM.MesiDoppioMercantile.Value : (short)0;
                        fondoPM.XPMMERGG = datiFondoPM.GiorniDoppioMercantile.HasValue ? datiFondoPM.GiorniDoppioMercantile.Value : (short)0;
                        fondoPM.XPMTERMM = datiFondoPM.MesiServizioMilitareATerra.HasValue ? datiFondoPM.MesiServizioMilitareATerra.Value : (short)0;
                        fondoPM.XPMTERGG = datiFondoPM.GiorniServizioMilitareATerra.HasValue ? datiFondoPM.GiorniServizioMilitareATerra.Value : (short)0;
                        fondoPM.XPMMACAA = datiFondoPM.TelegrafistaServizioMacchinaAA.HasValue ? datiFondoPM.TelegrafistaServizioMacchinaAA.Value : (short)0;
                        fondoPM.XPMMACMM = datiFondoPM.TelegrafistaServizioMacchinaMM.HasValue ? datiFondoPM.TelegrafistaServizioMacchinaMM.Value : (short)0;
                        fondoPM.XPMCODIF = datiFondoPM.CodiceDifferimentoPrivilegiato.HasValue ? datiFondoPM.CodiceDifferimentoPrivilegiato.Value : (short)0;
                        fondoPM.XPMDIFAA = datiFondoPM.PeriodoDifferimento1.HasValue ? datiFondoPM.PeriodoDifferimento1.Value : (short)0;
                        fondoPM.XPMDIFMM = datiFondoPM.PeriodoDifferimento2.HasValue ? datiFondoPM.PeriodoDifferimento2.Value : (short)0;
                        
                        if (!string.IsNullOrEmpty(datiFondoPM.GestioneSpeciale1Supplemento))
                        {
                            decimal val1;
                            if (!decimal.TryParse(datiFondoPM.GestioneSpeciale1Supplemento, out val1))
                                val1 = 0m;
                            fondoPM.XPMSUPP1 = val1;
                        }
                        else
                        {
                            fondoPM.XPMSUPP1 = 0m;
                        ...
                        if (!string.IsNullOrEmpty(datiFondoPM.GestioneSpeciale2Supplemento))
                        {
                            decimal val2;
                            if (!decimal.TryParse(datiFondoPM.GestioneSpeciale2Supplemento, out val2))
                                val2 = 0m;
                            fondoPM.XPMSUPP2 = val2;
                        }
                        else
                        {
                            fondoPM.XPMSUPP2 = 0m;
                        ...
                        fondoPM.XPMTILIQ = datiFondoPM.TipoLiquidazione.HasValue ? datiFondoPM.TipoLiquidazione.Value : (short)0;
                        fondoPM.XPMDIAGO = (datiFondoPM.DirittoAgo.HasValue && datiFondoPM.DirittoAgo.Value) ? (short)1 : (short)0;
                        fondoPM.XPMANULT = (datiFondoPM.AnnoUtileUltimoDecennio.HasValue && datiFondoPM.AnnoUtileUltimoDecennio.Value) ? (short)1 : (short)0;

                        fondoPM.XPMDPCRT = datiFondoPM.RMSDPCM161289.HasValue ? datiFondoPM.RMSDPCM161289.Value : 0m;
                        fondoPM.XPMS72RT = datiFondoPM.RMS7290.HasValue ? datiFondoPM.RMS7290.Value : 0m;

                        // Nuovi campi
                        fondoPM.XPMCORIP = datiFondoPM.CorresponsioneIP.HasValue ? datiFondoPM.CorresponsioneIP.Value : (short)0;
                        fondoPM.XPMRIPMM = datiFondoPM.MesiRiparametrazione.HasValue ? datiFondoPM.MesiRiparametrazione.Value : (short)0;
                        fondoPM.XPMRIPAA = datiFondoPM.AnniRiparametrazione.HasValue ? datiFondoPM.AnniRiparametrazione.Value : (short)0;
                        fondoPM.XPMESCLU = datiFondoPM.CodiceEsclusione.HasValue ? datiFondoPM.CodiceEsclusione.Value : (short)0;
                        fondoPM.XPMSTATO = datiFondoPM.Stato.HasValue ? datiFondoPM.Stato.Value : (short)0;
                        fondoPM.XPMRENDI = datiFondoPM.Rendimento.HasValue ? datiFondoPM.Rendimento.Value : 0m;
                        fondoPM.XPMDPCDC = datiFondoPM.CodiceDPCDC.HasValue ? datiFondoPM.CodiceDPCDC.Value : 0;
                    }
                    ...
                    if (listaDatiServizioUtile != null && listaDatiServizioUtile.Count > 0)
                    {
                        fondoPM.XPMSERVA = listaDatiServizioUtile[0].ServizioUtileAA.GetValueOrDefault();
                        fondoPM.XPMSERVM = listaDatiServizioUtile[0].ServizioUtileMM.GetValueOrDefault();
                        fondoPM.XPMRETPN = listaDatiServizioUtile[0].RetribuzionePensionabile.HasValue ? listaDatiServizioUtile[0].RetribuzionePensionabile.Value : 0M;
                    }
        ...
        public static void ValorizzaAgoEL(GestionePensione.DatiPensione datiPensione, GestioneFondo.DatiFondo datiFondo,
            List<GestioneRecordFondo.DatiRecordFondo> listaRecordFondo, GestioneDL407.DatiDL407 datiDL407, GestioneDanteCausa.DatiDanteCausa datiDanteCausa, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            ...
            if (datiFondo != null)
            {
                ...
                if (Utility.IsDomandaOrganizzazioniInternazionali(datiPensione)) // Memo 79
                    agoEL.YELSETDIR += datiFondo.SettimaneUtiliDirittoOI.HasValue ? datiFondo.SettimaneUtiliDirittoOI.Value : (int)0;
        ...
        public static void ValorizzaAgoTT(GestionePensione.DatiPensione datiPensione, GestioneFondo.DatiFondo datiFondo,
            List<GestioneRecordFondo.DatiRecordFondo> listaRecordFondo, GestioneDanteCausa.DatiDanteCausa datiDanteCausa, Object objectFondoXX, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            DateTime? decorrenzaPensioneOrDecorrenzaPensioneDC = Utility.GetDecorrenzaPensioneOrDecorrenzaDantecausa(datiPensione.DecorrenzaOriginaria, datiDanteCausa != null ? datiDanteCausa.DecorrenzaPensione : null);
            if (Utility.IsDomandaReversibilitaOrRicostituzione(datiPensione, datiDanteCausa) &&
            ...
            if (datiFondo != null)
            {
                ...
                if (Utility.IsDomandaOrganizzazioniInternazionali(datiPensione)) // Memo 79
                    agoTT.YTTSETDIR += datiFondo.SettimaneUtiliDirittoOI.HasValue ? datiFondo.SettimaneUtiliDirittoOI.Value : (int)0;
        ...
        public static void ValorizzaAgoET(GestionePensione.DatiPensione datiPensione, GestioneFondo.DatiFondo datiFondo, Object objectFondoXX, List<GestioneRecordFondo.DatiRecordFondo> listaRecordFondo,
            GestioneDanteCausa.DatiDanteCausa datiDanteCausa, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            DateTime? decorrenzaPensioneOrDecorrenzaPensioneDC = Utility.GetDecorrenzaPensioneOrDecorrenzaDantecausa(datiPensione.DecorrenzaOriginaria, datiDanteCausa != null ? datiDanteCausa.DecorrenzaPensione : null);
            if (Utility.IsDomandaReversibilitaOrRicostituzione(datiPensione, datiDanteCausa) &&
            ...
            if (datiFondo != null)
            {
                ...
                if (Utility.IsDomandaOrganizzazioniInternazionali(datiPensione)) // Memo 79
                    agoET.YETSETDIR += datiFondo.SettimaneUtiliDirittoOI.HasValue ? datiFondo.SettimaneUtiliDirittoOI.Value : (int)0;
            ...
            GestioneDanteCausa.DatiDanteCausa datiDanteCausa, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            ...
            if (datiFondo != null)
            {
                ...
                if (Utility.IsDomandaOrganizzazioniInternazionali(datiPensione)) // Memo 79
                    agoVL.YVLSETDIR += datiFondo.SettimaneUtiliDirittoOI.HasValue ? datiFondo.SettimaneUtiliDirittoOI.Value : (int)0;
                // 2015-01-22 G.Arru - Al momento non verrà mappato perchè non abbiamo le specifiche
                //agoVL.YVLCALC707    
            }
        ...
        public static void ValorizzaAgoFS(GestionePensione.DatiPensione datiPensione, GestioneFondo.DatiFondo datiFondo, GestioneIstruttoria.DatiIstruttoria datiIstruttoria,
            ...
            bool isNuovaGestione, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            if (listaRecordFondo != null && listaRecordFondo.Count > 0)
            {
        ...
        public static void ValorizzaAgoPT(GestionePensione.DatiPensione datiPensione, GestioneFondo.DatiFondo datiFondo, GestioneIstruttoria.DatiIstruttoria datiIstruttoria, List<GestioneRecordFondo.DatiRecordFondo> listaRecordFondo,
            List<GestioneDatiServizioUtile.ServizioUtile> listaDatiServizioUtile, Object objectFondoXX, bool isNuovaGestione, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            if (listaRecordFondo != null && listaRecordFondo.Count > 0)
            {
        ...
        public static void ValorizzaAgoES(GestionePensione.DatiPensione datiPensione, GestioneFondo.DatiFondo datiFondo,
            List<GestioneRecordFondo.DatiRecordFondo> listaRecordFondo, Object objectFondoXX, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
        ...
        {
            short i = 0;
            var lstAgoPM = new List<Data.CMSGTRA.Ago.PM>();
            List<GestioneFondo.DatiAgoPM> lstDatiAgoPM;
            GestioneFondo.GetListaDatiAgoPMByIdPensione(datiPensione.Id, out lstDatiAgoPM);
            GestioneCalcolo.DatiCalcoloContributivo calcoloContributivo = null;
            GestioneCalcolo.GetCalcoloContributivoByIdPensione(datiPensione.Id, out calcoloContributivo);
            GestioneCalcolo.DatiCalcoloRetributivo calcoloRetributivo = null;
            GestioneCalcolo.GetCalcoloRetributivoByIdPensione(datiPensione.Id, out calcoloRetributivo);

            if (lstDatiAgoPM != null && lstDatiAgoPM.Count > 0)
            {
                AreaCalcolo.AreaInputVariabile.ListaAgoPM = new List<Data.CMSGTRA.Ago.PM>();
                foreach (GestioneFondo.DatiAgoPM datiAgoPM in lstDatiAgoPM)
                {

                    var pm = new Data.CMSGTRA.Ago.PM();
                    i = (short)(i + 1);
                    pm.YPMFONDO = "PM";
                    pm.YPMTIPOR = "Y";
                    pm.YPMPROGR = i;
                    pm.YPMTIPEN = GetTipoPensione(datiPensione, datiFondo.TipoPensione);

                    if (datiAgoPM.TipoLiquidazione.HasValue)
                        pm.YPMTIPLQ = datiAgoPM.TipoLiquidazione.Value;

                    if (datiAgoPM.DecorrenzaContributiva != null)
                    {
                        pm.YPMDECAA = (short)datiAgoPM.DecorrenzaContributiva.Value.Year;
                        pm.YPMDECMM = (short)datiAgoPM.DecorrenzaContributiva.Value.Month;
                    }

                    if (datiAgoPM.SospensionePensione != null)
                    {
                        pm.YPMSCAAA = (short)datiAgoPM.SospensionePensione.Value.Year;
                        pm.YPMSCAMM = (short)datiAgoPM.SospensionePensione.Value.Month;
                    }

                    if (datiAgoPM.DecorrenzaReversibileAgo != null)
                    {
                        pm.YPMTEOAA = (short)datiAgoPM.DecorrenzaReversibileAgo.Value.Year;
                        pm.YPMTEOMM = (short)datiAgoPM.DecorrenzaReversibileAgo.Value.Month;
                    }

                    //Retributivi
                    //per il momento, se è il primo record inviamo quello salvato in questo modo
                    if (calcoloRetributivo != null && i == 1)
                    {
                        pm.YPMANZE1 = calcoloRetributivo.NSettimaneEsclusiveQuotaB.HasValue ? (short)calcoloRetributivo.NSettimaneEsclusiveQuotaB.Value : (short)0;
                        pm.YPMANZES = calcoloRetributivo.NSettimaneEsclusiveQuotaA.HasValue ? calcoloRetributivo.NSettimaneEsclusiveQuotaA.Value : 0;
                        pm.YPMANZT1 = calcoloRetributivo.NSettimaneQuotaB.HasValue ? (short)calcoloRetributivo.NSettimaneQuotaB.Value : (short)0;
                        pm.YPMANZTO = calcoloRetributivo.NSettimaneQuotaA.HasValue ? calcoloRetributivo.NSettimaneQuotaA.Value : 0;
                        pm.YPMRETP1 = calcoloRetributivo.RMSQuotaB.HasValue ? calcoloRetributivo.RMSQuotaB.Value : 0M;
                        pm.YPMRETPN = calcoloRetributivo.RMSQuotaA.HasValue ? calcoloRetributivo.RMSQuotaA.Value : 0M;
                    }
                    else
                    {
                        if (datiAgoPM.RMSQuotaA.HasValue)
                            pm.YPMRETPN = datiAgoPM.RMSQuotaA.Value;

                        if (datiAgoPM.NSettimaneQuotaA.HasValue)
                            pm.YPMANZTO = datiAgoPM.NSettimaneQuotaA.Value;

                        if (datiAgoPM.RMSQuotaB.HasValue)
                            pm.YPMRETP1 = datiAgoPM.RMSQuotaB.Value;

                        if (datiAgoPM.NSettimaneQuotaB.HasValue)
                            pm.YPMANZT1 = (short)datiAgoPM.NSettimaneQuotaB.Value;

                        if (datiAgoPM.NSettimaneEsclusiveQuotaB.HasValue)
                            pm.YPMANZE1 = (short)datiAgoPM.NSettimaneEsclusiveQuotaB.Value;

                        if (datiAgoPM.NsettimaneEsclusiveQuotaA.HasValue)
                            pm.YPMANZES = datiAgoPM.NsettimaneEsclusiveQuotaA.Value;
                    }
                    //----

                    if (datiAgoPM.NSettimaneAnzianitaVV.HasValue)
                        pm.YPMSETVV = datiAgoPM.NSettimaneAnzianitaVV.Value;

                    if (datiAgoPM.ImportoContrLegge3351995.HasValue)
                        pm.YPMCTRTO = datiAgoPM.ImportoContrLegge3351995.Value;

                    if (datiAgoPM.ImportoContrLegge3771958art24.HasValue)
                        pm.YPMCTRES = datiAgoPM.ImportoContrLegge3771958art24.Value;

                    if (datiAgoPM.ImportoContrSupLegge14321971art14.HasValue)
                        pm.YPMC14TO = datiAgoPM.ImportoContrSupLegge14321971art14.Value;

                    if (datiAgoPM.BaseAltraPensione.HasValue)
                        pm.YPMC14ES = datiAgoPM.BaseAltraPensione.Value;

                    if (datiAgoPM.ImportoDPR4881968art11.HasValue)
                        pm.YPMC11TO = datiAgoPM.ImportoDPR4881968art11.Value;

                    if (datiAgoPM.ImportoContEsclusiviSupDPR4881968art11.HasValue)
                        pm.YPMC11ES = datiAgoPM.ImportoContEsclusiviSupDPR4881968art11.Value;

                    if (!string.IsNullOrEmpty(datiAgoPM.YPMANNIR))
                    {
                        short annir;
                        if (short.TryParse(datiAgoPM.YPMANNIR, out annir))
                            pm.YPMANNIR = annir;
                    }

                    if (datiAgoPM.DecDPCM16121989art2 != null)
                    {
                        pm.YPMCDCAA = (short)datiAgoPM.DecDPCM16121989art2.Value.Year;
                        pm.YPMCDCMM = (short)datiAgoPM.DecDPCM16121989art2.Value.Month;
                    }

                    if (datiAgoPM.RMSDPCM.HasValue)
                        pm.YPMDPCRT = datiAgoPM.RMSDPCM.Value;

                    if (datiAgoPM.EtaMaturazioneRequisiti.HasValue)
                        pm.YPMETAMA = (short)datiAgoPM.EtaMaturazioneRequisiti.Value;

                    pm.YPMSPECI = datiAgoPM.CodiceSpecificoLiquidazioneAgo;

                   

                    pm.YPM503ET = datiAgoPM.YPM503ET;

                    if (!string.IsNullOrEmpty(datiAgoPM.YPM503AS))
                    {
                        short as503;
                        if (short.TryParse(datiAgoPM.YPM503AS, out as503))
                            pm.YPM503AS = as503;
                    }

                    pm.YPMTPCOD = datiAgoPM.YPMTPCOD;

                    if (!string.IsNullOrEmpty(datiAgoPM.YPMDECSS))
                    {
                        short decss;
                        if (short.TryParse(datiAgoPM.YPMDECSS, out decss))
                            pm.YPMDECSS = decss;
                    }

                    if (!string.IsNullOrEmpty(datiAgoPM.YPMSOSSS))
                    {
                        short sosss;
                        if (short.TryParse(datiAgoPM.YPMSOSSS, out sosss))
                            pm.YPMSOSSS = sosss;
                    }

                    pm.YPMAUTON = datiAgoPM.YPMAUTON;

                    //Contributivi
                    //per il momento, se è il primo record inviamo quello salvato in questo modo
                    if (calcoloContributivo != null && i == 1)
                    {
                        pm.YPMIMPCRT = calcoloContributivo.ImportoContribTotaleQuotaDL214.HasValue ? calcoloContributivo.ImportoContribTotaleQuotaDL214.Value : 0M;
                        pm.YPMMONTA = calcoloContributivo.Montante.HasValue ? calcoloContributivo.Montante.Value : 0M;
                        pm.YPMMONTA2012 = calcoloContributivo.MontanteQuotaDL214.HasValue ? calcoloContributivo.MontanteQuotaDL214.Value : 0M;
                        pm.YPMSETT2012 = calcoloContributivo.NSettimaneQuotaDL214.HasValue ? (short)calcoloContributivo.NSettimaneQuotaDL214.Value : (short)0;
                        pm.YPMSETTE = calcoloContributivo.NSettimane.HasValue ? (short)calcoloContributivo.NSettimane.Value : (short)0;
                    }
                    else
                    {
                        if (datiAgoPM.MontanteLegge3351995.HasValue) //
                            pm.YPMMONTA = datiAgoPM.MontanteLegge3351995.Value;

                        if (datiAgoPM.NSettimaneContributive.HasValue)
                            pm.YPMESCLU = datiAgoPM.NSettimaneContributive.Value;

                        if (datiAgoPM.ImportoContrQuotaD.HasValue) //
                            pm.YPMIMPCRT = datiAgoPM.ImportoContrQuotaD.Value;

                        if (datiAgoPM.MontanteQuotaD.HasValue) //
                            pm.YPMMONTA2012 = datiAgoPM.MontanteQuotaD.Value;

                        if (datiAgoPM.NSettimaneContributiveQuotaD.HasValue) //
                            pm.YPMSETT2012 = (short)datiAgoPM.NSettimaneContributiveQuotaD.Value;
                    }
                    //---

                    if (datiAgoPM.NSettimane707A.HasValue)
                        pm.YPMSETA_707 = (short)datiAgoPM.NSettimane707A.Value;

                    if (datiAgoPM.NSettimane707B.HasValue)
                        pm.YPMSETB_707 = (short)datiAgoPM.NSettimane707B.Value;

                    pm.YPMCALC707 = datiAgoPM.YPMCALC707;

                    if (!string.IsNullOrEmpty(datiAgoPM.YPMPROGR))
                    {
                        short progr;
                        if (short.TryParse(datiAgoPM.YPMPROGR, out progr))
                            pm.YPMPROGR = progr;
                    }

                    AreaCalcolo.AreaInputVariabile.ListaAgoPM.Add(pm);
            ...
            {
                //se non sono presenti i nuovi dati ago, utilizziamo vecchia gestione
        ...
        public static void ValorizzaAgoINPDAP(GestionePensione.DatiPensione datiPensione, GestioneFondo.DatiFondo datiFondo, GestioneIstruttoria.DatiIstruttoria datiIstruttoria, List<GestioneRecordFondo.DatiRecordFondo> listaRecordFondo,
           List<GestioneDatiServizioUtileINPDAP.ServizioUtile> listaDatiServizioUtile, Object objectFondoXX, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            if (listaRecordFondo != null && listaRecordFondo.Count > 0)
            {
        ...
        public static void ValorizzaAgoPI(GestionePensione.DatiPensione datiPensione, GestioneFondo.DatiFondo datiFondo, List<GestioneRecordFondo.DatiRecordFondo> listaRecordFondo,
          ref Data.FSPL_FSRCNew AreaCalcolo)
        {

            int i = 0;
            var lstAgoPI = new List<Data.CMSGTRA.Ago.PI>();
            List<GestioneFondo.DatiAgoPI> lstDatiAgoPI;
            GestioneFondo.GetListaDatiAgoPIByIdPensione(datiPensione.Id, out lstDatiAgoPI);

            if (lstDatiAgoPI != null && lstDatiAgoPI.Count > 0)
            {
                AreaCalcolo.AreaInputVariabile.ListaAgoPI = new List<Data.CMSGTRA.Ago.PI>();
                foreach (GestioneFondo.DatiAgoPI datiAgoPI in lstDatiAgoPI)
                {
                    Data.CMSGTRA.Ago.PI agoPI = new Data.CMSGTRA.Ago.PI();
                    if (datiPensione.SiglaCategoria != null && (datiPensione.SiglaCategoria.Trim() == "IPL" || datiPensione.SiglaCategoria.Trim() == "SPL"))
                        agoPI.YPIFONDO = "PL";
                    else
                        agoPI.YPIFONDO = "PI";
                    ...
                    agoPI.YPICTRES = datiAgoPI.Ctres.HasValue ? (decimal)datiAgoPI.Ctres : (decimal)0;
                    agoPI.YPIQUOFI = datiAgoPI.DirittoQuoteFisse.HasValue ? (short)datiAgoPI.DirittoQuoteFisse : (short)0;

                    i = i + 1;
                    agoPI.YPIPROGR = (short)i;
                    agoPI.YPITPENS = GetTipoPensione(datiPensione, datiFondo.TipoPensione);

                    if (datiAgoPI.DecorrenzaDatiAgo != null && datiAgoPI.DecorrenzaDatiAgo != DateTime.MinValue)
                    {
                        agoPI.YPIDECAA = (short)datiAgoPI.DecorrenzaDatiAgo.Value.Year;
                        agoPI.YPIDECMM = (short)datiAgoPI.DecorrenzaDatiAgo.Value.Month;
                    }

                    if (datiAgoPI.NSettimaneExCombattente.HasValue)
                        agoPI.YPINZTOT = datiAgoPI.NSettimaneExCombattente.Value;

                    agoPI.YPISPECI = datiAgoPI.CodiceSpecificoAgo;
                    agoPI.YPITPAGO = datiAgoPI.TipoLiquidazione.HasValue ? (short)datiAgoPI.TipoLiquidazione : (short)0;

                    if (datiAgoPI.SospensioneAgo != null && datiAgoPI.SospensioneAgo != DateTime.MinValue)
                    {
                        agoPI.YPISOSAA = (short)datiAgoPI.SospensioneAgo.Value.Year;
                        agoPI.YPISOSMM = (short)datiAgoPI.SospensioneAgo.Value.Month;
                    }

                    if (!string.IsNullOrEmpty(datiAgoPI.CodiceNatura) && datiAgoPI.CodiceNatura.Length >= 3)
                    {
                        agoPI.YPIALTR1 = (short)char.GetNumericValue(datiAgoPI.CodiceNatura[0]);
                        agoPI.YPIALTR2 = datiAgoPI.CodiceNatura[1].ToString();
                        agoPI.YPIALTR3 = datiAgoPI.CodiceNatura[2].ToString();
                    }

                    agoPI.YPISETVV = datiAgoPI.SettimaneVV.HasValue ? (short)datiAgoPI.SettimaneVV : (short)0;

                    agoPI.YPIS72RT = datiAgoPI.RMSRetributiva.HasValue ? (decimal)datiAgoPI.RMSRetributiva : (decimal)0;

                    // Quota A
                    agoPI.YPIRTSET = datiAgoPI.RMSQuotaA.HasValue ? (decimal)datiAgoPI.RMSQuotaA : (decimal)0;
                    agoPI.YPISZTOT = datiAgoPI.NSettimaneQuotaA.HasValue ? (int)datiAgoPI.NSettimaneQuotaA : (int)0;
                    agoPI.YPISZESC = datiAgoPI.NSettimaneEsclusiveQuotaA.HasValue ? (int)datiAgoPI.NSettimaneEsclusiveQuotaA : (int)0;

                    // Quota B
                    agoPI.YPIRSETB = datiAgoPI.RMSQuotaB.HasValue ? (decimal)datiAgoPI.RMSQuotaB : (decimal)0;
                    agoPI.YPISTOTB = datiAgoPI.NSettimaneQuotaB.HasValue ? (int)datiAgoPI.NSettimaneQuotaB : (int)0;
                    agoPI.YPINESCB = datiAgoPI.NSettimaneEsclusiveQuotaB.HasValue ? (int)datiAgoPI.NSettimaneEsclusiveQuotaB : (int)0;

                    // Montante
                    agoPI.YPICTRTT = datiAgoPI.Montante.HasValue ? (decimal)datiAgoPI.Montante : (decimal)0;

                    agoPI.YPISZRET = datiAgoPI.RMSQuotaAOmogenea.HasValue ? (decimal)datiAgoPI.RMSQuotaAOmogenea : (decimal)0;
                    agoPI.YPISRETB = datiAgoPI.RMSQuotaBOmogenea.HasValue ? (decimal)datiAgoPI.RMSQuotaBOmogenea : (decimal)0;
                    agoPI.YPICARIC = datiAgoPI.CausaCarico;
                    lstAgoPI.Add(agoPI);

                    AreaCalcolo.AreaInputVariabile.ListaAgoPI.Add(agoPI);
                    AreaCalcolo.Request.LISTBLOCCO.Add(new Data.HostRequest.FSPL_FSRCRequest.BLOCCO(AreaCalcolo.AreaInputVariabile.ListaAgoPI[0].YPITIPOR));
                }
            }

            //if (lstAgoPI != null && lstAgoPI.Count > 0)
            //{
            //    AreaCalcolo.AreaInputVariabile.ListaAgoPI = lstAgoPI;
            //    AreaCalcolo.Request.LISTBLOCCO.Add(new Data.HostRequest.FSPL_FSRCRequest.BLOCCO(AreaCalcolo.AreaInputVariabile.ListaAgoPI[0].YPITIPOR));
            //}
        }

        public static void ValorizzaAgoTeoricoPI(GestionePensione.DatiPensione datiPensione, GestioneFondo.DatiFondo datiFondo, List<GestioneRecordFondo.DatiRecordFondo> listaRecordFondo, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            var lstAgoTeorico = new List<Data.CMSGTRA.AgoTeorico>();

            List<GestioneFondo.DatiAgoTeoricoPI> lstDatiAgoTeoricoPI;
            GestioneFondo.GetListaDatiAgoTeoricoPIByIdPensione(datiPensione.Id, out lstDatiAgoTeoricoPI);

            if (lstDatiAgoTeoricoPI != null && lstDatiAgoTeoricoPI.Count > 0)
            {
                AreaCalcolo.AreaInputVariabile.ListaAgoTeorico = new List<Data.CMSGTRA.AgoTeorico>();
                foreach (GestioneFondo.DatiAgoTeoricoPI datiAgoTeoricoPI in lstDatiAgoTeoricoPI)
                {
                    Data.CMSGTRA.AgoTeorico agoTeorico = new INPS.Pensioni.LiquidazioneFs.Data.CMSGTRA.AgoTeorico();

                    agoTeorico.TRNTIPOR = "N";

                    // Tipo pensione
                    agoTeorico.TRNTPENS =
                        GetTipoPensione(datiPensione, datiFondo.TipoPensione);

                    // Decorrenza dati AGO teorico
                    if (datiAgoTeoricoPI.DecorrenzaDatiAgoTeorico != null &&
                        datiAgoTeoricoPI.DecorrenzaDatiAgoTeorico != DateTime.MinValue)
                    {
                        agoTeorico.TRNDECAA =
                            (short)datiAgoTeoricoPI.DecorrenzaDatiAgoTeorico.Value.Year;
                        agoTeorico.TRNDECMM =
                            (short)datiAgoTeoricoPI.DecorrenzaDatiAgoTeorico.Value.Month;
                    }

                    // Tipo liquidazione
                    agoTeorico.TRNTIPLQ =
                        datiAgoTeoricoPI.TipoLiquidazione.HasValue
                            ? (short)datiAgoTeoricoPI.TipoLiquidazione
                            : (short)0;

                    // Sospensione AGO teorica
                    if (datiAgoTeoricoPI.SospensioneAGOTeorica != null &&
                        datiAgoTeoricoPI.SospensioneAGOTeorica != DateTime.MinValue)
                    {
                        agoTeorico.TRNSOSAA =
                            (short)datiAgoTeoricoPI.SospensioneAGOTeorica.Value.Year;
                        agoTeorico.TRNSOSMM =
                            (short)datiAgoTeoricoPI.SospensioneAGOTeorica.Value.Month;
                    }

                    // Quota A
                    agoTeorico.TRNRSETA =
                        datiAgoTeoricoPI.RMSQuotaA.HasValue
                            ? (decimal)datiAgoTeoricoPI.RMSQuotaA
                            : (decimal)0;

                    agoTeorico.TRNSTOTA =
                        datiAgoTeoricoPI.NSettimaneTotaliQuotaA.HasValue
                            ? (int)datiAgoTeoricoPI.NSettimaneTotaliQuotaA
                            : (int)0;

                    agoTeorico.TRNSESCA =
                        datiAgoTeoricoPI.NSettimaneEsclusiveQuotaA.HasValue
                            ? (int)datiAgoTeoricoPI.NSettimaneEsclusiveQuotaA
                            : (int)0;

                    agoTeorico.TRNTEORA =
                        datiAgoTeoricoPI.RMSOmogeneaQuotaA.HasValue
                            ? (decimal)datiAgoTeoricoPI.RMSOmogeneaQuotaA
                            : (decimal)0;

                    // Quota B
                    agoTeorico.TRNRSETB =
                        datiAgoTeoricoPI.RMSQuotaB.HasValue
                            ? (decimal)datiAgoTeoricoPI.RMSQuotaB
                            : (decimal)0;

                    agoTeorico.TRNSTOTB =
                        datiAgoTeoricoPI.NSettimaneTotaliQuotaB.HasValue
                            ? (int)datiAgoTeoricoPI.NSettimaneTotaliQuotaB
                            : (int)0;

                    agoTeorico.TRNSESCB =
                        datiAgoTeoricoPI.NSettimaneEsclusiveQuotaB.HasValue
                            ? (int)datiAgoTeoricoPI.NSettimaneEsclusiveQuotaB
                            : (int)0;

                    agoTeorico.TRNTEORB =
                        datiAgoTeoricoPI.RMSOmogeneaQuotaB.HasValue
                            ? (decimal)datiAgoTeoricoPI.RMSOmogeneaQuotaB
                            : (decimal)0;

                    lstAgoTeorico.Add(agoTeorico);

                    AreaCalcolo.AreaInputVariabile.ListaAgoTeorico.Add(agoTeorico);
                    AreaCalcolo.Request.LISTBLOCCO.Add(new Data.HostRequest.FSPL_FSRCRequest.BLOCCO(AreaCalcolo.AreaInputVariabile.ListaAgoTeorico[0].TRNTIPOR));
                }

                //if (lstAgoTeorico != null && lstAgoTeorico.Count > 0)
                //{
                //    AreaCalcolo.AreaInputVariabile.ListaAgoTeorico = lstAgoTeorico;
                //    AreaCalcolo.Request.LISTBLOCCO.Add(new Data.HostRequest.FSPL_FSRCRequest.BLOCCO(AreaCalcolo.AreaInputVariabile.ListaAgoTeorico[0].TRNTIPOR));
                //}
            }
        }
        ...
        public static void ValorizzaMiglioramentiContrattuali(List<GestioneMiglioramentiContrattuali.DatiQuoteMiglioramentiContrattuali> datiQuoteMiglioramentiContrattuali, ref Data.FSPL_FSRCNew AreaCalcolo)
        {
            Data.CMSGTRA.MiglioramentiContrattuali miglioramentiContrattuali = null;

            if (datiQuoteMiglioramentiContrattuali != null && datiQuoteMiglioramentiContrattuali.Count > 0)
            {
                miglioramentiContrattuali = new INPS.Pensioni.LiquidazioneFs.Data.CMSGTRA.MiglioramentiContrattuali();
                miglioramentiContrattuali.TRSTPREC = "S";
                miglioramentiContrattuali.LISTWAUMENTICONTR = new List<Data.CMSGTRA.MiglioramentiContrattuali.W_AUMENTI_CONTR>();
                AreaCalcolo.AreaInputVariabile.ListaMiglioramentiContrattuali = new List<Data.CMSGTRA.MiglioramentiContrattuali>();

                foreach (var quota in datiQuoteMiglioramentiContrattuali)
                {
                    miglioramentiContrattuali.LISTWAUMENTICONTR.Add(new Data.CMSGTRA.MiglioramentiContrattuali.W_AUMENTI_CONTR()
                    {
                        WS_GP2BB05N = quota.Codice,
                        WS_GP2BB04DT = !string.IsNullOrEmpty(quota.DataDecorrenza) ? Convert.ToInt32(quota.DataDecorrenza) : 0,
                        WS_GP2BB06E = !string.IsNullOrEmpty(quota.Quota) ? decimal.Parse(quota.Quota) : 0
                    });
                }
                AreaCalcolo.AreaInputVariabile.ListaMiglioramentiContrattuali.Add(miglioramentiContrattuali);
                AreaCalcolo.Request.LISTBLOCCO.Add(new Data.HostRequest.FSPL_FSRCRequest.BLOCCO(AreaCalcolo.AreaInputVariabile.ListaMiglioramentiContrattuali[0].TRSTPREC));
            }
        }
