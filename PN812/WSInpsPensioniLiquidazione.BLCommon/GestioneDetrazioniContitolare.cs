...
{
    public class GestioneDetrazioniContitolare
    {
        public static void GetDetrazioniBySoggetto(long idPensione, long idAnagrafica, out DatiDetrazioniContitolare datiDetrazioni)
        {
            DetrazioniImpostaContitolare detrazioni = null;
            datiDetrazioni = null;
            DAGestioneDetrazioniContitolare.GetDetrazioniImpostaContitolareBySoggetto(idPensione, idAnagrafica, out detrazioni);
            if (detrazioni == null)
                return;
            datiDetrazioni = new DatiDetrazioniContitolare();
            Utility.ValorizzaOggetti(detrazioni, datiDetrazioni);
        }

        public static void GetDetrazioniByIdPensione(long idPensione, out List<DatiDetrazioniContitolare> listaDatiDetrazioni)
        {
            List<DetrazioniImpostaContitolare> listaDetrazioni = null;
            listaDatiDetrazioni = null;
            DAGestioneDetrazioniContitolare.GetDetrazioniImpostaContitolareByIdPensione(idPensione, out listaDetrazioni);
            if (listaDetrazioni == null)
                return;
            listaDatiDetrazioni = new List<DatiDetrazioniContitolare>();
            foreach (var detrazione in listaDetrazioni)
            {
                DatiDetrazioniContitolare datiDetrazioni = new DatiDetrazioniContitolare();
                Utility.ValorizzaOggetti(detrazione, datiDetrazioni);
                listaDatiDetrazioni.Add(datiDetrazioni);
            }
        }

        public static void SalvaDetrazioni(long idPensione, long idAnagrafica, DatiDetrazioniContitolare datiDetrazioni)
        {
            using (TransactionScope transactionScope = TransactionScopeFactory.Create(TransactionScopeOption.Required,
                new TransactionOptions { IsolationLevel = IsolationLevel.ReadCommitted }))
            {
                DetrazioniImpostaContitolare detrazioni = new DetrazioniImpostaContitolare();
                Utility.ValorizzaOggetti(datiDetrazioni, detrazioni);
                detrazioni.IdPensione = idPensione;
                detrazioni.IdAnagrafica = idAnagrafica;
                DAGestioneDetrazioniContitolare.SalvaDetrazioniImposta(detrazioni);
                transactionScope.Complete();
            }
        }

        public static void EliminaDetrazioniBySoggetto(long idPensione, long idAnagrafica)
        {
            using (TransactionScope transactionScope = TransactionScopeFactory.Create(TransactionScopeOption.Required,
                new TransactionOptions { IsolationLevel = IsolationLevel.ReadCommitted }))
            {
                DAGestioneDetrazioniContitolare.EliminaDetrazioniImpostaContitolareBySoggetto(idPensione, idAnagrafica);
                transactionScope.Complete();
            }
        }

        public static void EliminaDetrazioniByIdPensione(long idPensione)
        {
            using (TransactionScope transactionScope = TransactionScopeFactory.Create(TransactionScopeOption.Required,
                new TransactionOptions { IsolationLevel = IsolationLevel.ReadCommitted }))
            {
                DAGestioneDetrazioniContitolare.EliminaDetrazioniImpostaContitolareByIdPensione(idPensione);
                transactionScope.Complete();
            }
        }

        #region nested class
        public class DatiDetrazioniContitolare
        {
            public DatiDetrazioniContitolare()
            { }
            public DatiDetrazioniContitolare(long? idAnagrafica, byte? detrazioniReddito, byte? agevolazionePensionati, byte? coniugeOFiglio,
            ...
            public DatiDetrazioniContitolare(string detrazioni)
            {
            ...
            {
                DatiDetrazioniContitolare detr = (DatiDetrazioniContitolare)obj;
                try
                {
                    if (this.IdAnagrafica != detr.IdAnagrafica ||
                        this.AddizionaleLombardiaVeneto != detr.AddizionaleLombardiaVeneto ||
        ...
        public class DatiDetrazioniContitolareRecuperato : DatiDetrazioniContitolare
        {
