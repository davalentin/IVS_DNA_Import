...
{
    /// <summary>
    /// Summary description for EsitoCalcoloTest
    /// </summary>
    [TestClass]
    public class EsitoCalcoloTest
    {
        public EsitoCalcoloTest()
        {
        }
        ...
        [TestMethod]
        public void TestGetSaveCancelEsitoCalcoloByDomanda()
        {
            long idPensione = 0;
            GestionePensione.GetIdPensioneByNumeroDomanda(2038536800044, null, out idPensione);

            GestioneEsitoCalcolo.DatiEsitoCalcolo datiEsitoCalcolo = null;
            GestioneEsitoCalcolo.GetEsitoCalcoloByIdPensione(idPensione, out datiEsitoCalcolo);

            Assert.IsTrue(datiEsitoCalcolo == null, "DatiEsitoCalcolo non valorizzati correttamente");
            datiEsitoCalcolo = new GestioneEsitoCalcolo.DatiEsitoCalcolo("OK", "TUTTO OK");

            GestioneEsitoCalcolo.SalvaEsitoCalcolo(idPensione, datiEsitoCalcolo);

            datiEsitoCalcolo = null;
            GestioneEsitoCalcolo.GetEsitoCalcoloByIdPensione(idPensione, out datiEsitoCalcolo);
            Assert.IsTrue(datiEsitoCalcolo != null &&
            datiEsitoCalcolo.Esito == "OK" &&
            datiEsitoCalcolo.DettaglioEsito == "TUTTO OK", "Salvataggio DatiEsitoCalcolo non riuscito correttamente");
            
            GestioneEsitoCalcolo.EliminaEsitoCalcoloByIdPensione(idPensione);
