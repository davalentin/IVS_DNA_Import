...
using System.Linq;
using System.Text;
using INPS.Pensioni.LiquidazioneCi.ServiceReferences.NACI;
...
namespace INPS.Pensioni.LiquidazioneCi
{
    public class GestioneNACI
    {
        ...
        public static bool GetListaStatiIstituzione(long numDomanda, string matricola, short codiceSede, short centroOperativo, out List<GestioneContrib.PrestazioneEstera> listaPrestazioniEstere, out string cittadinanzaTitolare, out string errori)
        {
            errori = string.Empty;
            ...
            if (dati != null && dati.ModuloGetDatiNaciRisposta != null && dati.ModuloGetDatiNaciRisposta.DatiRisposta != null)
                NormalizzaDatiIstituzione(dati.ModuloGetDatiNaciRisposta.DatiRisposta, out listaPrestazioniEstere, out cittadinanzaTitolare, out errori);

            return true;
        }

        private static void NormalizzaDatiIstituzione(TipoGetDatiNaciRisposta dati, out List<GestioneContrib.PrestazioneEstera> listaPrestazioniEstere, out string cittadinanzaTitolare, out string errori)
        {
            listaPrestazioniEstere = null;
            cittadinanzaTitolare = string.Empty;
            errori = string.Empty;
            ...
            if (dati != null)
            {
                ...
                {
                    listaPrestazioniEstere = new List<GestioneContrib.PrestazioneEstera>();
                        ...
                        if (!string.IsNullOrEmpty(codStatoIstituzione))
                        {
                            ...
                            if (descPrestazioneEstera != null)
                            {
                                ...
                                listaPrestazioniEstere.Add(new INPS.Pensioni.LiquidazioneCi.GestioneContrib.PrestazioneEstera(descPrestazioneEstera.CDSTAIST, descPrestazioneEstera.SIGLISTI,
                                    ...
                                    !String.IsNullOrEmpty(stato.Matricola) ? stato.Matricola.Trim() : "", dati.DecisioneItaliana != null && !string.IsNullOrEmpty(dati.DecisioneItaliana.Codice) ? dati.DecisioneItaliana.Codice[0] : (char?)null, false));
                            }
                        }
                    }
                }

                if (!string.IsNullOrEmpty(dati.Cittadinanza) && dati.Cittadinanza.Trim() != string.Empty)
                {
                    string siglaNazione = dati.Cittadinanza.Trim().ToUpperInvariant();
                    ...
                    {
                        if (new List<string> { "I", "IT" }.Contains(siglaNazione))
                            siglaNazione = "ITA";

                        List<GestioneDecodifica.StatoEstero> statiEsteri = listaStatiEsteri.FindAll(x => x.Sigla.Trim().ToUpperInvariant() == siglaNazione);
