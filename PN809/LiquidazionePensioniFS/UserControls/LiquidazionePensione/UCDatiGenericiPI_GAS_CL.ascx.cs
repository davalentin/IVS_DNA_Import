...
{
    public partial class UCDatiGenericiPI_GAS_CL : CustomBaseUserControl, ILiquidazionePensione, ITitolarePensione
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            BindClick(this.domanda.Tipofondo);
            AddInputClass();
        }
        ...
        internal void ValorizzaEtichetteDatiGenerici(ILiquidazionePensione liquidazione, AreaTitolare.DatiPensione datiPensione, bool IsDomandaSperDonna)
        {
            ...
            enteFondo.Value = liquidazione.areaLiquidazionePensioneFS.CategoriaFondoPI != null ? liquidazione.areaLiquidazionePensioneFS.CategoriaFondoPI.ToString() : string.Empty;
            ...
            if (this.domanda.Tipofondo.HasValue)
            {
                switch (this.domanda.Tipofondo.Value)
                {
                    ...
                    case AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.PL:
                        ValorizzaEtichetteFromPensioneFondoPI(IsDomandaSperDonna, liquidazione);
                        break;
                    case AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.GAS:
                        ValorizzaEtichetteFromPensioneFondoGAS(IsDomandaSperDonna, liquidazione, datiPensione);
                        break;
            ...
            {
                //ddlCodNatura3DG.SelectedValue = "H"; //attualmente commentato. In attesa di capire se anche per il fondo GAS è presente il 3° codice natura H
        ...
        internal DatiGenerici GetDatiGenerici(AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo? tipoFondo)
        {
            ...
            switch (tipoFondo)
            {
                case AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.PI:
                case AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.PL:
                    areaLiquidazionePensioneFS.DatiGenerici = GetDatiGenericiToPensioneFondoPI(areaLiquidazionePensioneFS.DatiGenerici);
                    break;
                case AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.GAS:
                    areaLiquidazionePensioneFS.DatiGenerici = GetDatiGenericiToPensioneFondoGAS(areaLiquidazionePensioneFS.DatiGenerici);
                    break;
                case AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.CL:
                    break;
            }
        ...
        internal void ManageCodiceNoCalcoloPIU(List<RecordFondo> listaRecordFondo)
        {
            ...
            {
                if (listaRecordFondo != null && listaRecordFondo.Exists(x => x._CodiceNonCalcolo == 'S'))
                {
                    bool IsDomandaSperDonna = CodeUtility.IsDomandaSperimentaleDonna(datiPensione);

                    if (IsDomandaSperDonna || hdnIsTipoCalcoloEnabled.Value == "NO")
                    {
        ...
        private void BindClick(AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo? tipoFondo)
        {
        ...
        private void ValorizzaEtichetteCommon(bool IsDomandaSperDonna, ILiquidazionePensione liquidazione, AreaTitolare.DatiPensione datiPensione)
        {
            ...
            if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici != null)
            {
                ...
                {
                    if (this.domanda.Tipofondo.HasValue && this.domanda.Tipofondo.Value != AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.CL &&
                ...
                if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.RequisitiAl1294.HasValue)
                {
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.RequisitiAl1294.Value)
                        chkReqAnz1294.Checked = true;
                    else
                        chkReqAnz1294.Checked = false;
                }

                if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.RequisitiAl996.HasValue)
                {
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.RequisitiAl996.Value)
                        chkReqVecch996.Checked = true;
                    else
                        chkReqVecch996.Checked = false;
                }

                if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.RequisitiVecchiaiaAl1294.HasValue)
                {
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.RequisitiVecchiaiaAl1294.Value)
                ...
                if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.ExCombattente.HasValue)
                {
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.ExCombattente.Value)
                        chkExCombattente.Checked = true;
                    else
                        chkExCombattente.Checked = false;
                }

                if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.TrasformazioneAOI.HasValue)
                {
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.TrasformazioneAOI.Value)
                        chkTrasfAOI.Checked = true;
                    else
                ...
                {
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.ChkDL407.Value)
                        chkDL407.Checked = true;
                    else
                        chkDL407.Checked = false;
                }
            ...
            {
                {
                    ...
                    if (itemTipoCalcolo != null)
                    {
                        ...
                        {
                            ddlTipoCalcolo.Enabled = false;
                            hdnIsTipoCalcoloEnabled.Value = "NO";
                        }
                    }
                }
            }
        ...
        private DatiGenerici GetDatiGenericiCommon()
        {
            ...
            areaLiquidazionePensioneFS.DatiGenerici.TipoCalcolo = !String.IsNullOrEmpty(ddlTipoCalcolo.SelectedValue.Trim()) ? byte.Parse(ddlTipoCalcolo.SelectedValue) : (byte?)null;
        ...
        private void RenderControlsCommon(bool IsDomandaSperDonna, ILiquidazionePensione liquidazione, AreaTitolare.DatiPensione datiPensione)
        {
            ...
            if (liquidazione.areaLiquidazionePensioneFS.IsBeneficioArt24Comma15BisFromFELPE.GetValueOrDefault() ||
                ...
                datiPensione.IsDomandaQuota100OrRicostituzione || datiPensione.IsDomandaInabilitaAmiantoOrRicostituzione || datiPensione.SceltaLavoratriciMadri.HasValue || datiPensione.IsDomandaQuota102OrRicostituzione || datiPensione.IsDomandaAnticipataFlessibileOrRicostituzione
                || datiPensione.IsDomandaAnticipataFlessibileOpzioneContributivoOrRicostituzione || (!CodeUtility.IsRicostituzioneOrRiapertura(datiPensione, this.domanda.IsDomandaRiapertura) && (datiPensione.IsDomandaAnticipataFlessibileLeggeBilancio2024OrRicostituzione || datiPensione.IsDomandaAnticipataFlessibileLeggeBilancio2024OpzioneContributivoOrRicostituzione))
                || (CodeUtility.IsRicostituzioneOrRiapertura(datiPensione, this.domanda.IsDomandaRiapertura) && ((ViewState["AbilitazioneRIC_TRFMemo123_2024"] != null && (string)ViewState["AbilitazioneRIC_TRFMemo123_2024"].ToString().Trim().ToUpperInvariant() == "SI" && datiPensione.IsDomandaAnticipataFlessibileLeggeBilancio2024OrRicostituzione)
                || (ViewState["AbilitazioneRIC_TRFMemo123_2024OpzioneContrib"] != null && (string)ViewState["AbilitazioneRIC_TRFMemo123_2024OpzioneContrib"].ToString().Trim().ToUpperInvariant() == "SI" && datiPensione.IsDomandaAnticipataFlessibileLeggeBilancio2024OpzioneContributivoOrRicostituzione))))
        ...
        private void RenderControlsFromTipoFondo(ILiquidazionePensione liquidazione, AreaTitolare.DatiPensione datiPensione)
        {
            if (this.domanda.Tipofondo.HasValue)
            {
                switch (this.domanda.Tipofondo.Value)
                {
                    ...
                    case AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.PL:
                        if (ViewState[EnumViewState.CategoriaFondoPI.ToString()] != null &&
                            ((UtilityCategoriaFondoPI?)ViewState[EnumViewState.CategoriaFondoPI.ToString()] == UtilityCategoriaFondoPI.U ||
                             (UtilityCategoriaFondoPI?)ViewState[EnumViewState.CategoriaFondoPI.ToString()] == UtilityCategoriaFondoPI.V))
                        {
                            pnlTipoCalcolo.Visible = true;
                            if ((UtilityCategoriaFondoPI?)ViewState[EnumViewState.CategoriaFondoPI.ToString()] == UtilityCategoriaFondoPI.U)
                            {
                                pnlCheckExCombattente.Visible = true;
                                lblChkDL407.Visible = false;
                                chkDL407.Visible = false;
                                if (ViewState[EnumViewState.IsPnlCheckExCombattenteVisible.ToString()] != null &&
                                    ((bool)ViewState[EnumViewState.IsPnlCheckExCombattenteVisible.ToString()]) == false)
                                {
                                    pnlCheckExCombattente.Visible = false;
                                }
                            }
                        }
                        pnlINPDAP.Visible = true;
                        ManageCodiceNoCalcoloPIU(liquidazione.areaLiquidazionePensioneFS.ListaRecordFondo != null ? liquidazione.areaLiquidazionePensioneFS.ListaRecordFondo.ToList() : null);
                        if (this.domanda.Tipofondo.Value == AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.PL)
                        {
                            pnlTipoCalcolo.Visible = true;
                            pnlCheckExCombattente.Visible = true;
                        }
                    ...
                    case AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.GAS:
                        pnlTipoCalcolo.Visible = true;
                        pnlCheckMaggiorazioniBenefici.Visible = true;
                        pnlCheckExCombattente.Visible = true;
                        break;
                    case AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.CL:
                        pnlTipoCalcolo.Visible = true;
                        ...
                        pnlCheckMaggiorazioniBenefici.Visible = true;
                        pnlCheckExCombattente.Visible = false;
                        pnlCheckTrasformazioneAOI.Visible = false;
                        break;
                }
            }
        }
        ...
        private void LoadDdlFromTipoFondo(AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo? tipoFondo, ILiquidazionePensione liquidazione)
        {
            ...
            switch (tipoFondo)
            {
                ...
                case AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.PL:
                    ...
                    break;
                case AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.GAS:
                //Codici natura editabili. Non prevede il record blank nel primo codice natura.  
                case AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.CL:
                    break;
            }
            ...
            if (liquidazione.areaLiquidazionePensioneFS.ListaCodiciNatura != null)
            {
        ...
        private void ValorizzaEtichetteFromPensioneFondoPI(bool IsDomandaSperDonna, ILiquidazionePensione liquidazione)
        {
            if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici != null)
            {
                if (pnlRequisitiAnte247.Visible)
                {
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI.Requisiti247_243.HasValue && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI.Requisiti247_243.Value)
                        ddlReqAnte247.SelectedValue = "SI";
                    else if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI.Requisiti247_243.HasValue && !liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI.Requisiti247_243.Value)
                        ddlReqAnte247.SelectedValue = "NO";

                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI.NumeroTriSemRequisiti.HasValue)
                        ddlTrimestreRequisiti.SelectedValue = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI.NumeroTriSemRequisiti.ToString();

                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI.AnnoRequisiti.HasValue)
                        txtTrimestreRequisiti.Text = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI.AnnoRequisiti.ToString();
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI.AnzianitaAnni.HasValue)
                        txtAnzAnni.Text = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI.AnzianitaAnni.ToString();
                }
                else if (IsDomandaSperDonna)
                {
                    ...
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI.NumeroTriSemRequisiti.HasValue)
                        ddlSemestreRequisiti.SelectedValue = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI.NumeroTriSemRequisiti.ToString();

                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI.AnnoRequisiti.HasValue)
                        txtSemestreRequisiti.Text = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI.AnnoRequisiti.ToString();

                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI.AnzianitaAnni.HasValue)
                        txtAnzAnniSperDonna.Text = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPI.AnzianitaAnni.ToString();
                }
        ...
        private DatiGenerici GetDatiGenericiToPensioneFondoPI(DatiGenerici datiGenerici)
        {
            datiGenerici.fondoPI = new DatiGenerici.FondoPI();
            if (pnlRequisitiAnte247.Visible)
            {
                if (String.Equals(ddlReqAnte247.SelectedValue, "SI"))
                    datiGenerici.fondoPI.Requisiti247_243 = true;
                else if (String.Equals(ddlReqAnte247.SelectedValue, "NO"))
                    datiGenerici.fondoPI.Requisiti247_243 = false;
                if (!String.IsNullOrEmpty(ddlTrimestreRequisiti.SelectedValue))
                    datiGenerici.fondoPI.NumeroTriSemRequisiti = byte.Parse(ddlTrimestreRequisiti.SelectedValue);
                else if (!string.IsNullOrEmpty(hdnTrimesteRequisiti.Value))
                    datiGenerici.fondoPI.NumeroTriSemRequisiti = byte.Parse(hdnTrimesteRequisiti.Value);
                if ((!String.Equals(txtTrimestreRequisiti.Text.ToLowerInvariant(), "aaaa")) && (!(string.IsNullOrEmpty(txtTrimestreRequisiti.Text))))
                    datiGenerici.fondoPI.AnnoRequisiti = Int16.Parse(txtTrimestreRequisiti.Text);
                else if (!string.IsNullOrEmpty(hdnTrimesteRequisitiAnno.Value))
                    datiGenerici.fondoPI.AnnoRequisiti = Int16.Parse(hdnTrimesteRequisitiAnno.Value);
                if ((!String.Equals(txtAnzAnni.Text.ToLowerInvariant(), "aa")) && (!(string.IsNullOrEmpty(txtAnzAnni.Text))))
                    datiGenerici.fondoPI.AnzianitaAnni = Int32.Parse(txtAnzAnni.Text);
            }
            else if (this.pnlSperimentaleDonna.Visible)
            {
                ...
                if (!String.IsNullOrEmpty(ddlSemestreRequisiti.SelectedValue))
                    datiGenerici.fondoPI.NumeroTriSemRequisiti = byte.Parse(ddlSemestreRequisiti.SelectedValue);
                if ((!String.Equals(txtSemestreRequisiti.Text.ToLowerInvariant(), "aaaa")) && (!(string.IsNullOrEmpty(txtSemestreRequisiti.Text))))
                    datiGenerici.fondoPI.AnnoRequisiti = Int16.Parse(txtSemestreRequisiti.Text);
                if ((!String.Equals(txtAnzAnniSperDonna.Text.ToLowerInvariant(), "aa")) && (!(string.IsNullOrEmpty(txtAnzAnniSperDonna.Text))))
                    datiGenerici.fondoPI.AnzianitaAnni = Int32.Parse(txtAnzAnniSperDonna.Text);
            }
            else if (pnlTrimestreAnzianitaRequisitiNoInvalidita.Visible)
            {
                if (!String.IsNullOrEmpty(ddlTrimestreRequisiti.SelectedValue))
        ...
        public void ManageVisibilityPnlCheckExCombattente(bool visible)
        {
            if (!visible)
            {
                chkExCombattente.Checked = false; //sbianco la chkExCombattente
            }
            ViewState[EnumViewState.IsPnlCheckExCombattenteVisible.ToString()] = visible;
            pnlCheckExCombattente.Visible = visible;
        }
        #endregion PI

        #region GAS

        private void ValorizzaEtichetteFromPensioneFondoGAS(bool IsDomandaSperDonna, ILiquidazionePensione liquidazione, AreaTitolare.DatiPensione datiPensione)
        {
            if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici != null)
            {
                if (pnlRequisitiAnte247.Visible)
                {
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS.Requisiti247_243.HasValue && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS.Requisiti247_243.Value)
                        ddlReqAnte247.SelectedValue = "SI";
                    else if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS.Requisiti247_243.HasValue && !liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS.Requisiti247_243.Value)
                        ddlReqAnte247.SelectedValue = "NO";

                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS.NumeroTriSemRequisiti.HasValue)
                        ddlTrimestreRequisiti.SelectedValue = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS.NumeroTriSemRequisiti.ToString();

                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS.AnnoRequisiti.HasValue)
                        txtTrimestreRequisiti.Text = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS.AnnoRequisiti.ToString();
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS.AnzianitaAnni.HasValue)
                        txtAnzAnni.Text = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS.AnzianitaAnni.ToString();
                }
                else if (IsDomandaSperDonna)
                {
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS.Requisiti247_243.HasValue && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS.Requisiti247_243.Value)
                    ...
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS.NumeroTriSemRequisiti.HasValue)
                        ddlSemestreRequisiti.SelectedValue = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS.NumeroTriSemRequisiti.ToString();

                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS.AnnoRequisiti.HasValue)
                        txtSemestreRequisiti.Text = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoGAS.AnnoRequisiti.ToString();
        ...
        private DatiGenerici GetDatiGenericiToPensioneFondoGAS(DatiGenerici datiGenerici)
        {
            datiGenerici.fondoGAS = new DatiGenerici.FondoGAS();
            if (pnlRequisitiAnte247.Visible)
            {
                if (String.Equals(ddlReqAnte247.SelectedValue, "SI"))
                    datiGenerici.fondoGAS.Requisiti247_243 = true;
                else if (String.Equals(ddlReqAnte247.SelectedValue, "NO"))
                    datiGenerici.fondoGAS.Requisiti247_243 = false;
                if (!String.IsNullOrEmpty(ddlTrimestreRequisiti.SelectedValue))
                    datiGenerici.fondoGAS.NumeroTriSemRequisiti = byte.Parse(ddlTrimestreRequisiti.SelectedValue);
                else if (!string.IsNullOrEmpty(hdnTrimesteRequisiti.Value))
                    datiGenerici.fondoGAS.NumeroTriSemRequisiti = byte.Parse(hdnTrimesteRequisiti.Value);
                if ((!String.Equals(txtTrimestreRequisiti.Text.ToLowerInvariant(), "aaaa")) && (!(string.IsNullOrEmpty(txtTrimestreRequisiti.Text))))
                    datiGenerici.fondoGAS.AnnoRequisiti = Int16.Parse(txtTrimestreRequisiti.Text);
                else if (!string.IsNullOrEmpty(hdnTrimesteRequisitiAnno.Value))
                    datiGenerici.fondoGAS.AnnoRequisiti = Int16.Parse(hdnTrimesteRequisitiAnno.Value);
                if ((!String.Equals(txtAnzAnni.Text.ToLowerInvariant(), "aa")) && (!(string.IsNullOrEmpty(txtAnzAnni.Text))))
                    datiGenerici.fondoGAS.AnzianitaAnni = Int32.Parse(txtAnzAnni.Text);
            }
            else if (this.pnlSperimentaleDonna.Visible)
            {
                if (String.Equals(ddlSperimentaleDonna.SelectedValue, "SI"))
                    ...
                    datiGenerici.fondoGAS.Requisiti247_243 = false;
                if (!String.IsNullOrEmpty(ddlSemestreRequisiti.SelectedValue))
                    datiGenerici.fondoGAS.NumeroTriSemRequisiti = byte.Parse(ddlSemestreRequisiti.SelectedValue);
                if ((!String.Equals(txtSemestreRequisiti.Text.ToLowerInvariant(), "aaaa")) && (!(string.IsNullOrEmpty(txtSemestreRequisiti.Text))))
                    datiGenerici.fondoGAS.AnnoRequisiti = Int16.Parse(txtSemestreRequisiti.Text);
                if ((!String.Equals(txtAnzAnniSperDonna.Text.ToLowerInvariant(), "aa")) && (!(string.IsNullOrEmpty(txtAnzAnniSperDonna.Text))))
                    datiGenerici.fondoGAS.AnzianitaAnni = Int32.Parse(txtAnzAnniSperDonna.Text);
            }
            else if (pnlTrimestreAnzianitaRequisitiNoInvalidita.Visible)
            {
                if (!String.IsNullOrEmpty(ddlTrimestreRequisiti.SelectedValue))
        ...
        {
            CategoriaFondoPI,
            IsPnlCheckExCombattenteVisible,
            DatiStoricoGP
        }
        #endregion Enum
    }
