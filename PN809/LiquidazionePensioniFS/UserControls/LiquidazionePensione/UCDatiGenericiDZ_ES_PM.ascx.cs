...
{
    public partial class UCDatiGenericiDZ_ES_PM : CustomBaseUserControl, ILiquidazionePensione, ITitolarePensione
    {
            ...
            if (this.domanda.Tipofondo.HasValue)
            {
                switch (this.domanda.Tipofondo.Value)
                {
                    case AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.DZ:
                        ValorizzaEtichetteCommonFromPensioneFondoDZ(IsDomandaSperDonna, liquidazione);
                        break;
                    case AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.ES:
                        ValorizzaEtichetteCommonFromPensioneFondoES(IsDomandaSperDonna, liquidazione);
                        break;
                    case AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.PM:
                        RenderControlsFondoPM(liquidazione);
                        ValorizzaEtichetteCommonFromPensioneFondoPM(IsDomandaSperDonna, liquidazione, datiPensione);
                        break;
                }
            }
        ...
        internal DatiGenerici GetDatiGenerici(AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo? tipoFondo)
        {
            ...
            {
                case AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.DZ:
                    areaLiquidazionePensioneFS.DatiGenerici = GetDatiGenericiToPensioneFondoDZ(areaLiquidazionePensioneFS.DatiGenerici);
                    break;
                case AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.ES:
                    areaLiquidazionePensioneFS.DatiGenerici = GetDatiGenericiToPensioneFondoES(areaLiquidazionePensioneFS.DatiGenerici);
                    break;
                case AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.PM:
                    areaLiquidazionePensioneFS.DatiGenerici = GetDatiGenericiToPensioneFondoPM(areaLiquidazionePensioneFS.DatiGenerici);
                    break;
            }
        ...
        internal void SetDdlCodiceComunicazione3(AreaDecodifica datiDecodifica, ILiquidazionePensione liquidazione)
        {
            ...
            foreach (AreaDecodifica.DatiComunicazioneCampo3 comunicazioneCampo3 in datiDecodifica.ElencoComunicazioneCampo3)
            {
                switch (comunicazioneCampo3.Id)
                {
                    ...
                    default:
                        if (ViewState["IsProvvisoriaVisible"] != null && (bool)ViewState["IsProvvisoriaVisible"] &&
                            (liquidazione.areaLiquidazionePensioneFS != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici != null
                            && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.CodiceComunicazioneCampo3 != null
        ...
        private void ValorizzaEtichetteCommon(bool IsDomandaSperDonna, ILiquidazionePensione liquidazione, AreaTitolare.DatiPensione datiPensione)
        {
            ...
            if (this.domanda.Tipofondo.HasValue)
                hdnTipoFondo.Value = this.domanda.Tipofondo.ToString();
            ...
            if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici != null)
            {
                ...
                if (!(!String.IsNullOrEmpty(this.domanda.Categoria) && this.domanda.Categoria.Trim() == "VPM"))
                {
                ...
                //se il codiceComunicazione3 è valorizzato lo seleziono
                //if(areaLiquidazionePensioneFS.DatiGenerici.CodiceComunicazioneCampo3.HasValue)
                //    ddlCodComunicazioni3.SelectedValue = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.CodiceComunicazioneCampo3.ToString();
        ...
        private DatiGenerici GetDatiGenericiCommon()
        {
            ...
            if (!String.IsNullOrEmpty(ddlTipoCalcolo.SelectedValue))
                areaLiquidazionePensioneFS.DatiGenerici.TipoCalcolo = byte.Parse(ddlTipoCalcolo.SelectedValue);
        ...
        private void RenderControlsCommon(bool IsDomandaSperDonna, ILiquidazionePensione liquidazione, AreaTitolare.DatiPensione datiPensione)
        {
            ...
            if (this.domanda.Categoria.Trim() == "IDZ")
                this.pnlRequisitiVecchiaia.Enabled = false;
        ...
        private void ManageScadenzaRevSanitaria(AreaTitolare.DatiPensione datiPensione)
        {
            ...
            {
                if (this.domanda.Tipofondo.HasValue && this.domanda.Tipofondo.Value != AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.PM)
                ...
                switch (tipoFondo.Value)
                {
                    case AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.DZ:
                        ...
                        foreach (INPS.Pensioni.LiquidazionePensione.Presenter.SvrLiquidazioneFs.TipoCalcolo tipoCalcolo in liquidazione.areaLiquidazionePensioneFS.ListaTipoCalcolo)
                            if (tipoCalcolo.TraduzioneSuGP == 1)
                    ...
                    case AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.PM:
                        ...
                        ddlCodiceTipoLiquidazione.Items.Clear();
                        CodeUtility.SetItemBlankDdl(ddlCodiceTipoLiquidazione);
                        foreach (INPS.Pensioni.LiquidazionePensione.Presenter.SvrLiquidazioneFs.CodiceTipoLiquidazionePM tipoLiquidazione in liquidazione.areaLiquidazionePensioneFS.ListaCodiceTipoLiquidazionePM)
                        {
                            CodeUtility.SetValueDdl(ddlCodiceTipoLiquidazione, tipoLiquidazione.Id.ToString() + " - " + tipoLiquidazione.Descrizione, tipoLiquidazione.Descrizione, tipoLiquidazione.Id.ToString());
                        }
                        break;
                }
            }
        ...
        private void LoadDdlCommon(ILiquidazionePensione liquidazione, Presenter.SvrLiquidazione.AreaDecodifica datiDecodifica, AreaTitolare.DatiPensione datiPensione, AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo? tipoFondo)
        {
        ...
        private void ValorizzaEtichetteCommonFromPensioneFondoDZ(bool IsDomandaSperDonna, ILiquidazionePensione liquidazione)
        {
            lblRevisioneSanitaria.Text = "Scadenza Revisione Sanitaria:";
            if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici != null)
            {
                if (this.pnlRequisitiAnte247.Visible)
                {
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ.Requisiti247_243.HasValue && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ.Requisiti247_243.Value)
                        ddlReqAnte247.SelectedValue = "SI";
                    else if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ.Requisiti247_243.HasValue && !liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ.Requisiti247_243.Value)
                        ddlReqAnte247.SelectedValue = "NO";

                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ.NumeroTriSemRequisiti.HasValue)
                        ddlTrimestreRequisiti.SelectedValue = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ.NumeroTriSemRequisiti.ToString();

                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ.AnnoRequisiti.HasValue)
                        txtTrimestreRequisiti.Text = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ.AnnoRequisiti.ToString();
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ.AnzianitaAnni.HasValue)
                        txtAnzAnni.Text = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ.AnzianitaAnni.ToString();
                }
                else if (IsDomandaSperDonna)
                {
                    ...
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ.NumeroTriSemRequisiti.HasValue)
                        ddlSemestreRequisiti.SelectedValue = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ.NumeroTriSemRequisiti.ToString();

                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ.AnnoRequisiti.HasValue)
                        txtSemestreRequisiti.Text = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ.AnnoRequisiti.ToString();

                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ.AnzianitaAnni.HasValue)
                        txtAnzAnniSperDonna.Text = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ.AnzianitaAnni.ToString();
                }
        ...
        private DatiGenerici GetDatiGenericiToPensioneFondoDZ(DatiGenerici datiGenerici)
        {
            datiGenerici.fondoDZ = new DatiGenerici.FondoDZ();
            if (this.pnlRequisitiAnte247.Visible)
            {
                if (String.Equals(ddlReqAnte247.SelectedValue, "SI"))
                    datiGenerici.fondoDZ.Requisiti247_243 = true;
                else if (String.Equals(ddlReqAnte247.SelectedValue, "NO"))
                    datiGenerici.fondoDZ.Requisiti247_243 = false;
                if (!String.IsNullOrEmpty(ddlTrimestreRequisiti.SelectedValue))
                    datiGenerici.fondoDZ.NumeroTriSemRequisiti = byte.Parse(ddlTrimestreRequisiti.SelectedValue);
                else if (!string.IsNullOrEmpty(hdnTrimesteRequisiti.Value))
                    datiGenerici.fondoDZ.NumeroTriSemRequisiti = byte.Parse(hdnTrimesteRequisiti.Value);
                if ((!String.Equals(txtTrimestreRequisiti.Text.ToLowerInvariant(), "aaaa")) && (!(string.IsNullOrEmpty(txtTrimestreRequisiti.Text))))
                    datiGenerici.fondoDZ.AnnoRequisiti = Int16.Parse(txtTrimestreRequisiti.Text);
                else if (!string.IsNullOrEmpty(hdnTrimesteRequisitiAnno.Value))
                    datiGenerici.fondoDZ.AnnoRequisiti = Int16.Parse(hdnTrimesteRequisitiAnno.Value);
                if ((!String.Equals(txtAnzAnni.Text.ToLowerInvariant(), "aa")) && (!(string.IsNullOrEmpty(txtAnzAnni.Text))))
                    datiGenerici.fondoDZ.AnzianitaAnni = Int32.Parse(txtAnzAnni.Text);
            }
            else if (this.pnlSperimentaleDonna.Visible)
            {
                ...
                if (!String.IsNullOrEmpty(ddlSemestreRequisiti.SelectedValue))
                    datiGenerici.fondoDZ.NumeroTriSemRequisiti = byte.Parse(ddlSemestreRequisiti.SelectedValue);
                if ((!String.Equals(txtSemestreRequisiti.Text.ToLowerInvariant(), "aaaa")) && (!(string.IsNullOrEmpty(txtSemestreRequisiti.Text))))
                    datiGenerici.fondoDZ.AnnoRequisiti = Int16.Parse(txtSemestreRequisiti.Text);
                if ((!String.Equals(txtAnzAnniSperDonna.Text.ToLowerInvariant(), "aa")) && (!(string.IsNullOrEmpty(txtAnzAnniSperDonna.Text))))
                    datiGenerici.fondoDZ.AnzianitaAnni = Int32.Parse(txtAnzAnniSperDonna.Text);
            }
            else if (pnlTrimestreAnzianitaRequisitiNoInvalidita.Visible)
            {
                if (!String.IsNullOrEmpty(ddlTrimestreRequisiti.SelectedValue))
        ...
        #region ES
        private void ValorizzaEtichetteCommonFromPensioneFondoES(bool IsDomandaSperDonna, ILiquidazionePensione liquidazione)
        {
            lblRevisioneSanitaria.Text = "Data Revisione Sanitaria:";

            if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici != null)
            {
                if (this.pnlRequisitiAnte247.Visible)
                {
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES.Requisiti247_243.HasValue && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES.Requisiti247_243.Value)
                        ddlReqAnte247.SelectedValue = "SI";
                    else if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES.Requisiti247_243.HasValue && !liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES.Requisiti247_243.Value)
                        ddlReqAnte247.SelectedValue = "NO";

                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES.NumeroTriSemRequisiti.HasValue)
                        ddlTrimestreRequisiti.SelectedValue = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES.NumeroTriSemRequisiti.ToString();

                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES.AnnoRequisiti.HasValue)
                        txtTrimestreRequisiti.Text = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES.AnnoRequisiti.ToString();
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES.AnzianitaAnni.HasValue)
                        txtAnzAnni.Text = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES.AnzianitaAnni.ToString();
                }
                else if (IsDomandaSperDonna)
                {
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES.Requisiti247_243.HasValue && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES.Requisiti247_243.Value)
                    ...
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES.NumeroTriSemRequisiti.HasValue)
                        ddlSemestreRequisiti.SelectedValue = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES.NumeroTriSemRequisiti.ToString();

                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES.AnnoRequisiti.HasValue)
                        txtSemestreRequisiti.Text = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES.AnnoRequisiti.ToString();

                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES.AnzianitaAnni.HasValue)
                        txtAnzAnniSperDonna.Text = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES.AnzianitaAnni.ToString();
                }
                else if (pnlTrimestreAnzianitaRequisitiNoInvalidita.Visible)
                {
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoES.NumeroTriSemRequisiti.HasValue)
        ...
        private DatiGenerici GetDatiGenericiToPensioneFondoES(DatiGenerici datiGenerici)
        {
            datiGenerici.fondoES = new DatiGenerici.FondoES();
            if (this.pnlRequisitiAnte247.Visible)
            {
                if (String.Equals(ddlReqAnte247.SelectedValue, "SI"))
                    datiGenerici.fondoES.Requisiti247_243 = true;
                else if (String.Equals(ddlReqAnte247.SelectedValue, "NO"))
                    datiGenerici.fondoES.Requisiti247_243 = false;
                if (!String.IsNullOrEmpty(ddlTrimestreRequisiti.SelectedValue))
                    datiGenerici.fondoES.NumeroTriSemRequisiti = byte.Parse(ddlTrimestreRequisiti.SelectedValue);
                else if (!string.IsNullOrEmpty(hdnTrimesteRequisiti.Value))
                    datiGenerici.fondoES.NumeroTriSemRequisiti = byte.Parse(hdnTrimesteRequisiti.Value);
                if ((!String.Equals(txtTrimestreRequisiti.Text.ToLowerInvariant(), "aaaa")) && (!(string.IsNullOrEmpty(txtTrimestreRequisiti.Text))))
                    datiGenerici.fondoES.AnnoRequisiti = Int16.Parse(txtTrimestreRequisiti.Text);
                else if (!string.IsNullOrEmpty(hdnTrimesteRequisitiAnno.Value))
                    datiGenerici.fondoES.AnnoRequisiti = Int16.Parse(hdnTrimesteRequisitiAnno.Value);
                if ((!String.Equals(txtAnzAnni.Text.ToLowerInvariant(), "aa")) && (!(string.IsNullOrEmpty(txtAnzAnni.Text))))
                    datiGenerici.fondoES.AnzianitaAnni = Int32.Parse(txtAnzAnni.Text);
            }
            else if (this.pnlSperimentaleDonna.Visible)
            {
                if (String.Equals(ddlSperimentaleDonna.SelectedValue, "SI"))
                    ...
                    datiGenerici.fondoES.Requisiti247_243 = false;
                if (!String.IsNullOrEmpty(ddlSemestreRequisiti.SelectedValue))
                    datiGenerici.fondoES.NumeroTriSemRequisiti = byte.Parse(ddlSemestreRequisiti.SelectedValue);
                if ((!String.Equals(txtSemestreRequisiti.Text.ToLowerInvariant(), "aaaa")) && (!(string.IsNullOrEmpty(txtSemestreRequisiti.Text))))
                    datiGenerici.fondoES.AnnoRequisiti = Int16.Parse(txtSemestreRequisiti.Text);
                if ((!String.Equals(txtAnzAnniSperDonna.Text.ToLowerInvariant(), "aa")) && (!(string.IsNullOrEmpty(txtAnzAnniSperDonna.Text))))
                    datiGenerici.fondoES.AnzianitaAnni = Int32.Parse(txtAnzAnniSperDonna.Text);
            }
            else if (pnlTrimestreAnzianitaRequisitiNoInvalidita.Visible)
            {
                if (!String.IsNullOrEmpty(ddlTrimestreRequisiti.SelectedValue))
        ...
        #region PM
        private void ValorizzaEtichetteCommonFromPensioneFondoPM(bool IsDomandaSperDonna, ILiquidazionePensione liquidazione, AreaTitolare.DatiPensione datiPensione)
        {
            if (this.domanda == null)
                this.domanda = (Presenter.SvrLiquidazione.AreaRispostaRiepilogo.DatiRiepilogoDomanda)Session["Domanda"];
            ...
            if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici != null)
            {
                if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.ChkDL407.HasValue)
                    chkDL407.Checked = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.ChkDL407.Value;

                if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPM != null)
                {
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPM.CodiceTipoLiquidazione.HasValue)
                        ddlCodiceTipoLiquidazione.SelectedValue = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPM.CodiceTipoLiquidazione.Value.ToString();
                }

                //    if (this.pnlRequisitiNoInvalidita.Visible)
                //    {
                //        if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPM != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPM.Requisiti247_243.HasValue && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ.Requisiti247_243.Value)
                //            ddlReqAnte247.SelectedValue = "SI";
                //        else if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ.Requisiti247_243.HasValue && !liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ.Requisiti247_243.Value)
                //            ddlReqAnte247.SelectedValue = "NO";
                ...
                //            if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPM.NumeroTriSemRequisiti.HasValue)
                //                ddlSemestreRequisiti.SelectedValue = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoDZ.NumeroTriSemRequisiti.ToString();
                ...
                if (pnlTrimestreAnzianitaRequisitiNoInvalidita.Visible)
                {
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPM != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPM.NumeroTriSemRequisiti.HasValue)
                        ddlTrimestreRequisiti.SelectedValue = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPM.NumeroTriSemRequisiti.ToString();

                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPM != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPM.AnnoRequisiti.HasValue)
                        txtTrimestreRequisiti.Text = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPM.AnnoRequisiti.ToString();
                    if (liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPM != null && liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPM.AnzianitaAnni.HasValue)
                        txtAnzAnni.Text = liquidazione.areaLiquidazionePensioneFS.DatiGenerici.fondoPM.AnzianitaAnni.ToString();
                }
            ...
            ManagePostArmonizzazioniForPM(liquidazione);
        }

        public void ManagePostArmonizzazioniForPM(ILiquidazionePensione liquidazione)
        {
            if (liquidazione.areaLiquidazionePensioneFS.IsDecorrenzaSuccSett1989.HasValue && liquidazione.areaLiquidazionePensioneFS.IsDecorrenzaSuccSett1989.Value)
            {
                AreaTitolare.DatiPensione datiPensione = GetDatiPensione(this);
                AreaTitolare.DatiPensione.TipoDomanda tipoDomanda = datiPensione.Tipo;

                //Per le riaperture si avrà un comportamento uguale alle ricostituzioni
                ...
                switch (tipoDomanda)
                {
                    ...
                    case AreaTitolare.DatiPensione.TipoDomanda.Normale:
                        ddlCodNatura2DG.Visible = false;
                        ddlCodNatura3DG.Visible = false;
                        break;
                ...
                if (!String.IsNullOrEmpty(this.domanda.Categoria) && this.domanda.Categoria.Trim() == "VPM")
                {
                    ddlCodNatura1DG.SelectedValue = "1";
                    ddlCodNatura1DG.Enabled = false;
                }
            }
        }

        private DatiGenerici GetDatiGenericiToPensioneFondoPM(DatiGenerici datiGenerici)
        {
            datiGenerici.ChkDL407 = chkDL407.Checked;

            datiGenerici.fondoPM = new DatiGenerici.FondoPM();

            if (!string.IsNullOrEmpty(hdnCodiceTipoLiquidazione.Value))
                datiGenerici.fondoPM.CodiceTipoLiquidazione = byte.Parse(hdnCodiceTipoLiquidazione.Value);


            //if (!string.IsNullOrEmpty(ddlCodiceTipoLiquidazione.SelectedValue))
            //    datiGenerici.fondoPM.CodiceTipoLiquidazione = byte.Parse(ddlCodiceTipoLiquidazione.SelectedValue);
            ...
            {
                if (!String.IsNullOrEmpty(ddlTrimestreRequisiti.SelectedValue))
                    datiGenerici.fondoPM.NumeroTriSemRequisiti = byte.Parse(ddlTrimestreRequisiti.SelectedValue);
                if ((!String.Equals(txtTrimestreRequisiti.Text.ToLowerInvariant(), "aaaa")) && (!(string.IsNullOrEmpty(txtTrimestreRequisiti.Text))))
                    datiGenerici.fondoPM.AnnoRequisiti = Int16.Parse(txtTrimestreRequisiti.Text);
                if ((!String.Equals(txtAnzAnni.Text.ToLowerInvariant(), "aa")) && (!(string.IsNullOrEmpty(txtAnzAnni.Text))))
                    datiGenerici.fondoPM.AnzianitaAnni = Int32.Parse(txtAnzAnni.Text);
            }

            return datiGenerici;
        }

        private void RenderControlsFondoPM(ILiquidazionePensione liquidazione)
        {
            if (!liquidazione.areaLiquidazionePensioneFS.IsDecorrenzaSuccSett1989.GetValueOrDefault())
                pnlDL407.Visible = true;
            else
                pnlTrasfAOI.Visible = false;

            pnlTipoLiquidazione.Visible = true;
        }
