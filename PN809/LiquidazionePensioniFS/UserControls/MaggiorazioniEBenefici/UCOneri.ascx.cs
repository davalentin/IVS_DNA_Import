...
{
    public partial class UCOneri : CustomBaseUserControl, IMaggiorazioneBenefici, ITitolarePensione
    {
        ...
        internal void ValorizzaEtichetteOneri(IMaggiorazioneBenefici maggiorazioneBenefici)
        {
            ...
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (Utility.IsDomandaUnicarpe(datiPensione, true) == Utility.TipoUnicarpe.Lettura_L)
                    {
                        ...
                        {
                            ((Label)e.Row.FindControl("lblDecorrenza")).Text = String.Format("{0:dd/MM/yyyy}", ((DatiOneri)(e.Row.DataItem)).Decorrenza);
                            ((Label)e.Row.FindControl("lblCessazione")).Text = String.Format("{0:dd/MM/yyyy}", ((DatiOneri)(e.Row.DataItem)).Scadenza);
                        }
                        else
                        {
                    ...
                    else if (e.Row.Cells[0].Controls.Count == 3)
                    {
                            ...
                            this.domanda.Tipofondo.Value == AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.PT))
                        {
                            ...
                            ((Label)e.Row.FindControl("lblDecorrenza_Edit")).Text = String.Format("{0:dd/MM/yyyy}", ((DatiOneri)(e.Row.DataItem)).Decorrenza);
                            ((TextBox)e.Row.FindControl("txtCessazioneFS_PT")).Text = String.Format("{0:dd/MM/yyyy}", ((DatiOneri)(e.Row.DataItem)).Scadenza);
                        }
                        else
                        {
        ...
        protected void gvOneri_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            else if (e.CommandName == "Salva")
            {
                ...
                if (this.domanda.Tipofondo.HasValue && (this.domanda.Tipofondo.Value == AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.FS ||
                            this.domanda.Tipofondo.Value == AreaRispostaRiepilogo.DatiRiepilogoDomanda.TipoFondo.PT))
                {
                    elencoOneri[r.DataItemIndex].Decorrenza = Utility.ConvertString2Data(((Label)(r.Cells[3].Controls[1])).Text);
                    elencoOneri[r.DataItemIndex].Scadenza = Utility.ConvertString2Data(((TextBox)(r.Cells[4].FindControl("txtCessazioneFS_PT"))).Text);
                }
                else
                {
                    elencoOneri[r.DataItemIndex].Decorrenza = Utility.ConvertString2Data_MMAAAA(((Label)(r.Cells[3].Controls[1])).Text);
                    elencoOneri[r.DataItemIndex].Scadenza = Utility.ConvertString2Data_MMAAAA(((TextBox)(r.Cells[4].FindControl("txtCessazione"))).Text);
                }
                short resShort = 0;
