...
namespace INPS.Pensioni.LiquidazionePensione.View.Web
{
    ...
    public partial class SistemaUnico : BasePage, ISedi, IRicercaPosizione
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                Session.Add("SistemaUnico", true);
                Session["IsSistemaUnico"] = true;
                ...
                else
                {
                    ...
                    if (Session["Ruolo"] == null)
                    {
                        ...
                        if (ruoliAbilitati != null && ruoliAbilitati.Count(x => x.Key == "P8854") > 0) // AGO
                            Session["Ruolo"] = (Ruoli)Enum.Parse(typeof(Ruoli), "P8854");
                        else if (ruoliAbilitati != null && ruoliAbilitati.Count(x => x.Key == "P4677") > 0) // FS
                            Session["Ruolo"] = (Ruoli)Enum.Parse(typeof(Ruoli), "P4677");
                        else if (ruoliAbilitati != null && ruoliAbilitati.Count(x => x.Key == "P8855") > 0) // CI
                            Session["Ruolo"] = (Ruoli)Enum.Parse(typeof(Ruoli), "P8855");

                        if (Session["Ruolo"] == null)
                        {
                            if (CodeUtility.IsMultiRuolo())
                            {
                                Session.Add("UrlWebDom", HttpContext.Current.Request.Url.PathAndQuery);
                ...
                if (ConfigurationManager.AppSettings["WEBDOM-CLIENT"] != null &&
                    ConfigurationManager.AppSettings["WEBDOM-CLIENT"] == "1")
                {
                ...
                else if (ConfigurationManager.AppSettings["WEBDOM-CLIENT"] != null &&
                    ConfigurationManager.AppSettings["WEBDOM-CLIENT"] == "2")
                {
                    ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), "OpenModalDialog", "<script type='text/javascript'>document.getElementById('" + btnRicercaNDomus.ClientID + "').click() ;</script>", false);
                    btnRicercaNDomus.Style.Add("display", "none");
        ...
        private void RicercaNDomus(string numeroDomanda)
        {
            ...
            if (HasError && ErrorMessage == "Ruolo Utente non abilitato alla lavorazione della domanda.")
            {
                HasError = false;
                ErrorMessage = string.Empty;
                Session["Ruolo"] = Ruoli.P4677;
                this.TipoAppRuolo = (UtilityTipoAppartenenza)Utility.GetTipoAppartenenzaRuolo((Ruoli)Session["Ruolo"]);
                presenterElaborazionePosizione.RicercaDomanda(this);        //Chiamata a RicercaDomanda
            }
            ...
            else
            {
                ...
                {
                    throw new DnaApplicationException("Sistema Unico, Errore in btnRicercaNDomus_Click" + ex);
                }
